...
NEWS_CACHE_TTL_HOT_TOPICS = int(
    os.getenv("NEWS_CACHE_TTL_HOT_TOPICS", "120")
)  # 热点缓存默认2分钟
...
def _generate_news_id(
    source: str, title: str, publish_time: datetime | None = None
) -> str:
...
class NewsService:
    ...
    def __init__(self):
        ...
        self._fetch_semaphore = asyncio.Semaphore(
            int(os.getenv("NEWS_FETCH_MAX_CONCURRENCY", "4"))
        )
        self._request_interval = float(os.getenv("NEWS_REQUEST_INTERVAL", "1.0"))
        self._use_sina_fallback = os.getenv(
            "NEWS_ENABLE_SINA_FALLBACK", "true"
        ).lower() in {"1", "true", "yes"}
    ...
    def _set_cache(self, key: str, value: Any, ttl: int = 300):
        ...
        if redis:
            try:
                redis.setex(
                    f"{NEWS_CACHE_PREFIX}{key}", ttl, json.dumps(value, default=str)
                )
            except Exception as e:
    ...
    async def _run_tushare_with_retry(
        self, source_name: str, method_name: str, **kwargs
    ):
            ...
            try:
                async with self._fetch_semaphore:
                    df = await asyncio.to_thread(
                        getattr(self.tushare_pro, method_name), **kwargs
                    )
        ...
        raise RuntimeError(
            f"{source_name} failed after {max_retries} retries"
        ) from last_exc
    ...
    async def get_news_by_stock(
        ...
        if self._use_sina_fallback:
            try:
                sina_news = await self._get_sina_stock_news(
                    stock_code, max(limit // 2, 5)
                )
    ...
    async def get_market_news(
        ...
        """获取市场整体财经新闻。"""
        category_str = (
            category.value if isinstance(category, NewsCategory) else str(category)
        )
        ...
        if category == NewsCategory.ALL:
            ...
            task_results = await asyncio.gather(
                _safe_fetch(
                    "tushare_news",
                    self._fetch_tushare_news(start_dt, end_dt, limit=limit),
                ),
                _safe_fetch(
                    "tushare_major",
                    self._fetch_tushare_major_news(
                        start_dt, end_dt, limit=max(limit // 2, 10)
                    ),
                ),
                _safe_fetch(
                    "tushare_anns",
                    self._fetch_tushare_anns_d(
                        start_date=start_date, end_date=end_date, limit=max(limit, 20)
                    ),
                ),
                _safe_fetch(
                    "tushare_cctv",
                    self._fetch_tushare_cctv_news(now.strftime("%Y%m%d")),
                ),
                _safe_fetch(
                    "tushare_research",
                    self._fetch_tushare_research_report(
                        trade_date=now.strftime("%Y%m%d"), limit=max(limit, 20)
                    ),
                ),
                _safe_fetch(
                    "tushare_npr",
                    self._fetch_tushare_npr(
                        start_date=start_dt, end_date=end_dt, limit=max(limit // 2, 10)
                    ),
                ),
            )
            for group in task_results:
                news_items.extend(group)
            if self._use_sina_fallback and not any(
                item.source == "tushare_news" for item in news_items
            ):
                news_items.extend(
                    await _safe_fetch("sina", self._get_sina_finance_news(limit))
                )
        else:
            if category == NewsCategory.FLASH:
                flash_news = await _safe_fetch(
                    "tushare_news",
                    self._fetch_tushare_news(start_dt, end_dt, limit=limit),
                )
            ...
            if category == NewsCategory.ANALYSIS:
                news_items.extend(
                    await _safe_fetch(
                        "tushare_major",
                        self._fetch_tushare_major_news(
                            start_dt, end_dt, limit=max(limit, 20)
                        ),
                    )
                )
            ...
            if category in [NewsCategory.NPR, NewsCategory.POLICY]:
                news_items.extend(
                    await _safe_fetch(
                        "tushare_npr",
                        self._fetch_tushare_npr(
                            start_date=start_dt, end_date=end_dt, limit=max(limit, 20)
                        ),
                    )
                )
        ...
        result = sorted(
            deduped.values(), key=lambda x: x.publish_time or datetime.min, reverse=True
        )[:limit]
        ...
        self._set_cache(
            cache_key, [item.model_dump() for item in result], NEWS_CACHE_TTL_FLASH
        )
        self._set_fetch_meta(failed_sources)
    ...
    def _schedule_sentiment_analysis(
        ...
        try:
            asyncio.create_task(
                self._background_analyze_sentiment(missing, stock_context)
            )
        except Exception as e:
    ...
    async def search_news(
        ...
        matched = [
            ...
            or keyword_lower in news.content.lower()
        ]
    ...
    async def _fetch_tushare_major_news(
        ...
        for one_src in src_list:
            ...
            for _, row in df.head(limit).iterrows():
                ...
                items.append(
                    NewsItem(
                        id=_generate_news_id(
                            f"tushare_major:{src_name}", title, pub_time
                        ),
                        title=title,
                        content=content,
    ...
    async def _fetch_tushare_anns_d(
        ...
        """获取上市公司公告。"""
        kwargs: dict[str, Any] = {"fields": "ann_date,ts_code,name,title,url,rec_time"}
        if ts_code:
    ...
    async def _fetch_tushare_research_report(
        ...
        df = await self._run_tushare_with_retry(
            "tushare_research", "research_report", **kwargs
        )
        if df is None or df.empty:
            return []
        ...
        for _, row in df.head(limit).iterrows():
            ...
            content_parts = [
                ...
                for p in [
                    abstr,
                    f"机构: {inst_name}" if inst_name else "",
                    f"行业: {ind_name_val}" if ind_name_val else "",
                ]
                if p
            ]
            items.append(
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code != 200:
                    logger.warning(
                        f"Sina stock news request failed: {response.status_code}"
                    )
                    return []

                # 解析 HTML 提取新闻
                news_items = self._parse_sina_stock_news_html(
                    response.text, stock_code, limit
                )
                return news_items
    ...
    async def _get_sina_finance_news(
        ...
        try:
            ...
            async with httpx.AsyncClient(timeout=10.0) as client:
                ...
                if response.status_code != 200:
                    logger.warning(
                        f"Sina finance news request failed: {response.status_code}"
                    )
                    return []
...
只输出JSON，不要其他文字。"""
    ...
    def _build_summary_prompt(
        ...
        news_text = "\n".join(
            [
                f"- {item.title}"
                + (f": {item.content[:100]}..." if item.content else "")
...
请以JSON格式输出。"""
    ...
    def _simple_summary(self, news_items: list[NewsItem]) -> dict[str, Any]:
        ...
        return {
            "summary": f"共有 {len(news_items)} 条相关新闻。最新动态包括："
            + "；".join(titles),
            "key_points": titles[:3],
