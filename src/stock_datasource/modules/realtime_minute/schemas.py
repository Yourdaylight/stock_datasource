...
class MinuteBar(BaseModel):
    ...
    freq: str | None = Field(None, description="频率")


class MinuteDataResponse(BaseModel):
    """Response for minute data query."""

    ts_code: str | None = Field(None, description="证券代码")
...
class BatchMinuteDataResponse(BaseModel):
    ...
    data: dict[str, list[MinuteBar]] = Field(
        default_factory=dict, description="按代码分组的数据"
    )
...
class RankItem(BaseModel):
    ...
    close: float | None = Field(None, description="最新价")
    pct_chg: float | None = Field(None, description="涨跌幅(%)")
    vol: float | None = Field(None, description="成交量")
    amount: float | None = Field(None, description="成交额")
    trade_time: str | None = Field(None, description="数据时间")
    market_type: str | None = Field(None, description="市场类型")
...
class MarketOverviewResponse(BaseModel):
    ...
    total_vol: float | None = Field(None, description="总成交量")
    total_amount: float | None = Field(None, description="总成交额")
    avg_pct_chg: float | None = Field(None, description="平均涨跌幅")
...
class MarketStatsResponse(BaseModel):
    ...
    markets: dict[str, dict[str, int]] = Field(
        default_factory=dict, description="各市场分布"
    )
