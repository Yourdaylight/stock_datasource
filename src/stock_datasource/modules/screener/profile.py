...
from .schemas import ProfileDimension, StockProfile
...
def _calculate_percentile(
    value: float, values: list[float], reverse: bool = False
) -> float:
    """计算百分位排名 (0-100)"""
...
class ProfileService:
    ...
    def _get_valuation_data(self, ts_code: str, days: int = 60) -> list[dict[str, Any]]:
        ...
        try:
            end_date = datetime.now().strftime("%Y%m%d")
            start_date = (datetime.now() - timedelta(days=days * 2)).strftime("%Y%m%d")
            result = self.daily_basic_service.get_daily_basic(
    ...
    def calculate_volume_score(
        self, daily_data: list[dict[str, Any]], valuation_data: list[dict[str, Any]]
    ) -> ProfileDimension:
        """计算量能维度评分"""
        indicators = {}
        scores = []
    ...
    def calculate_risk_score(
        self, ts_code: str, stock_info: dict[str, Any]
    ) -> ProfileDimension:
        """计算风险维度评分 - 低风险得高分"""
        indicators = {}
        score = 80.0  # 默认分数
    ...
    def calculate_profile(self, ts_code: str) -> StockProfile | None:
        """计算股票十维画像"""
        try:
            # 获取股票基本信息
    ...
    def _generate_recommendation(
        self, total_score: float, dimensions: list[ProfileDimension]
    ) -> str:
        """生成投资建议"""
