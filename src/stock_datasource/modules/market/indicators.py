...
def _clean_nan(values: list | pd.Series) -> list[float | None]:
...
def calculate_rsi(df: pd.DataFrame, period: int = 14) -> dict[str, list[float | None]]:
    ...
    return {f"RSI{period}": _clean_nan(rsi.round(2))}
...
def calculate_atr(df: pd.DataFrame, period: int = 14) -> dict[str, list[float | None]]:
    ...
    return {f"ATR{period}": _clean_nan(atr.round(2))}


def calculate_obv(df: pd.DataFrame) -> dict[str, list[float | None]]:
    ...
    return {"OBV": _clean_nan(obv.round(0))}


def calculate_dmi(df: pd.DataFrame, period: int = 14) -> dict[str, list[float | None]]:
...
def calculate_cci(df: pd.DataFrame, period: int = 14) -> dict[str, list[float | None]]:
    ...
    return {f"CCI{period}": _clean_nan(cci.round(2).fillna(0))}
...
def calculate_indicators(
    df: pd.DataFrame, indicators: list[str], params: dict[str, Any] | None = None
) -> dict[str, Any]:
    ...
    for indicator in indicators:
        ...
        try:
            ...
            indicator_data = (
                calculator(df, **indicator_params)
                if indicator_params
                else calculator(df)
            )
...
def detect_signals(
    df: pd.DataFrame, indicators_data: dict[str, list[float]]
) -> list[dict[str, Any]]:
    ...
    if "DIF" in indicators_data and "DEA" in indicators_data:
        ...
        if len(dif) >= 2 and len(dea) >= 2:
            ...
            if _safe_compare(dif[-1], dea[-1], ">") and _safe_compare(
                dif[-2], dea[-2], "<="
            ):
                signals.append(
                    {
    ...
    # RSI signals
    rsi_keys = [k for k in indicators_data if k.startswith("RSI")]
