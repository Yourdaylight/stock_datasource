...
def ingest_daily_data(**context):
    ...
    try:
        ...
        result = ingestion_service.ingest_daily_data(
            trade_date=trade_date, run_quality_checks=True
        )
        ...
        logger.info(
            f"Tables processed: {len(result.get('loading', {}).get('tables_processed', []))}"
        )

        # Check if ingestion failed
        if result["status"] == "failed":
            raise Exception(
                f"Daily ingestion failed: {result.get('error', 'Unknown error')}"
            )
...
def generate_daily_report(**context):
    ...
    try:
        ...
        logger.info(
            f"Daily report generated: {report.get('summary', {}).get('overall_status', 'unknown')}"
        )
...
def cleanup_old_data(**context):
    ...
    try:
        ...
        logger.info(
            f"Cleanup completed: {len(result.get('tables_cleaned', []))} tables cleaned"
        )
