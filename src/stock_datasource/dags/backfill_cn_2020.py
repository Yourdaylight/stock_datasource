...
def run_historical_backfill(**context):
    ...
    try:
        ...
        result = ingestion_service.backfill_data(
            start_date=start_date, end_date=end_date, run_quality_checks=True
        )
...
def generate_backfill_report(**context):
    ...
    try:
        backfill_result = context["task_instance"].xcom_pull(
            task_ids="run_historical_backfill"
        )
        ...
        logger.info(
            f"Backfill report generated: {report.get('summary', {}).get('overall_status', 'unknown')}"
        )
