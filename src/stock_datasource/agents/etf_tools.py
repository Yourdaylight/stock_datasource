...
def calculate_etf_metrics(ts_code: str, days: int = 60) -> dict[str, Any]:
    ...
    avg_amount = (
        round(sum(amounts) / len(amounts) / 10000, 2) if amounts else 0
    )  # In 万元
...
def compare_etf_with_index(ts_code: str, days: int = 30) -> dict[str, Any]:
    ...
    if not benchmark:
        return {"ts_code": ts_code, "message": "该ETF未设置跟踪指数，无法进行对比分析"}
    ...
    return {
        ...
        "index_cum_return": round(index_cum_return, 2)
        if index_cum_return is not None
        else None,
        "tracking_diff": tracking_diff,
        "analysis": _generate_tracking_analysis(
            etf_cum_return, index_cum_return, tracking_diff
        ),
    }


def _generate_tracking_analysis(
    etf_return: float, index_return: float | None, diff: float | None
) -> str:
