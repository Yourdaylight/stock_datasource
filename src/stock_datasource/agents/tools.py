...
def get_stock_info(ts_code: str) -> str:
    ...
    try:
        ...
        lines.append(
            f"- 开盘: {row['open']:.2f} | 最高: {row['high']:.2f} | 最低: {row['low']:.2f}"
        )
        ...
        return f"查询股票 {ts_code} 信息失败: {e!s}"
...
def get_stock_kline(ts_code: str, days: int = 30) -> str:
    ...
    try:
        ...
        df = db.execute_query(
            query,
            {"code": ts_code, "start": start_date, "end": end_date, "limit": days},
        )
        ...
        lines.append(
            "| 日期 | 开盘 | 最高 | 最低 | 收盘 | 涨跌幅 | 成交量(万手) | 成交额(亿) |"
        )
        lines.append(
            "|------|------|------|------|------|--------|-------------|-----------|"
        )
        ...
        return f"查询 {ts_code} K线数据失败: {e!s}"
...
def get_stock_valuation(ts_code: str) -> str:
    ...
    try:
        ...
        return f"查询 {ts_code} 估值数据失败: {e!s}"
...
def calculate_technical_indicators(ts_code: str) -> str:
    ...
    try:
        ...
        """
        df = db.execute_query(
            query, {"code": ts_code, "start": start_date, "end": end_date}
        )
        ...
        return f"计算 {ts_code} 技术指标失败: {e!s}"
...
def _get_hk_stock_info(ts_code: str) -> str:
    ...
    try:
        ...
        if row.get("open") is not None:
            lines.append(
                f"- 开盘: {row['open']:.3f} | 最高: {row['high']:.3f} | 最低: {row['low']:.3f}"
            )
        ...
        return f"查询港股 {ts_code} 信息失败: {e!s}"
...
def _get_hk_stock_kline(ts_code: str, days: int = 30) -> str:
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            vol = (
                f"{row['volume'] / 10000:.2f}" if row.get("volume") is not None else "-"
            )
            lines.append(
                f"| {row['trade_date']} | {open_p} | {high_p} | {low_p} | {close_p} | {chg} | {vol} |"
            )
        ...
        return f"查询港股 {ts_code} K线数据失败: {e!s}"
...
def _calculate_hk_technical_indicators(ts_code: str) -> str:
    ...
    try:
        ...
        return f"计算港股 {ts_code} 技术指标失败: {e!s}"
...
def _get_hk_stock_valuation(ts_code: str) -> str:
    ...
    try:
        ...
        return (
            f'港股 {ts_code} 暂无独立估值数据表，建议使用"XX.HK 财报分析"获取财务指标'
        )
    except Exception as e:
...
def screen_stocks(
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            lines.append(
                f"| {row['ts_code']} | {row['name']} | {row.get('industry', '-')} | {pe} | {pb} | {mv} | {tr} |"
            )
...
def get_stock_profile(ts_code: str) -> str:
    ...
    try:
        ...
        for dim in profile.dimensions:
            lines.append(
                f"| {dim.name} | {dim.score:.1f} | {dim.level} | {dim.weight * 100:.0f}% |"
            )
...
def get_sector_stocks(sector: str, limit: int = 20) -> str:
    ...
    try:
        ...
        for _, row in df.iterrows():
            ...
            mv = f"{row['total_mv'] / 10000:.1f}" if row.get("total_mv") else "-"
            lines.append(
                f"| {row['ts_code']} | {row['name']} | {close} | {pct} | {pe} | {pb} | {mv} |"
            )
