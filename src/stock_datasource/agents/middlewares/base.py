...
class AgentContext:
    ...
    signal: Any | None = None  # SignalResult
    # Loop detection
...
class BaseMiddleware(ABC):
    ...
    async def before_with_logging(self, context: AgentContext) -> AgentContext:
        ...
        try:
            ...
            self._log.info(
                "middleware.before.done", middleware=self.name, duration_ms=ms
            )
            return result
        except Exception as e:
            self._log.error(
                "middleware.before.failed", middleware=self.name, error=str(e)
            )
            raise

    async def after_with_logging(
        self, context: AgentContext, response: AgentResponse
    ) -> AgentResponse:
        ...
        try:
            ...
            ms = round((time.time() - t0) * 1000, 1)
            self._log.info(
                "middleware.after.done", middleware=self.name, duration_ms=ms
            )
            return result
        except Exception as e:
            self._log.error(
                "middleware.after.failed", middleware=self.name, error=str(e)
            )
            raise
