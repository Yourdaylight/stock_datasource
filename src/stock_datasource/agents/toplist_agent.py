...
from .base_agent import AgentConfig, LangGraphAgent, ToolDefinition
...
注意保持客观中立，强调风险提示。"""
    ...
    async def get_institutional_analysis(self, trade_date: str) -> dict[str, Any]:
        """分析机构资金流向"""
        try:
            analysis = await self.analysis_service.analyze_institutional_flow(
                trade_date
            )
    ...
    async def analyze_stock_concentration(
        self, ts_code: str, days: int = 10
    ) -> dict[str, Any]:
        """分析股票席位集中度"""
        try:
            concentration = await self.analysis_service.calculate_seat_concentration(
                ts_code, days
            )
    ...
    async def analyze_sector_flows(self, trade_date: str) -> dict[str, Any]:
        """分析板块资金流向"""
        try:
            sector_analysis = await self.analysis_service.analyze_sector_flow(
                trade_date
            )
    ...
    async def generate_toplist_report(self, trade_date: str) -> dict[str, Any]:
        ...
        try:
            ...
            report = {
                ...
                "hot_money_targets": hot_money_targets.get("data", {}).get(
                    "targets", []
                )[:10],
    ...
    def _generate_key_insights(
        ...
        try:
            ...
            if sector_flows_list:
                ...
                insights.append(
                    f"{top_sector['sector']}板块资金流入最多，可能存在轮动机会"
                )
