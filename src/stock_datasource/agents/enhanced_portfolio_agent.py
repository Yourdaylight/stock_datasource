...
from .base_agent import AgentConfig, AgentResult, LangGraphAgent
...
class EnhancedPortfolioAgent(LangGraphAgent):
    ...
    def toplist_analysis_service(self):
        ...
        if self._toplist_analysis_service is None:
            try:
                from stock_datasource.services.toplist_analysis_service import (
    ...
    async def analyze_portfolio_performance(
        ...
        try:
            ...
            # Get profit history
            profit_history = await self.portfolio_service.get_profit_history(
            ...
            sector_analysis = self._analyze_sector_allocation(
                summary.sector_distribution or {}
            )
    ...
    async def analyze_individual_stock(
        self, ts_code: str, analysis_type: str = "comprehensive"
    ) -> dict[str, Any]:
        """分析单个股票的技术面和基本面."""
        try:
            analysis_result = {
    ...
    async def generate_investment_recommendations(
        ...
        try:
            ...
            for position in positions[:5]:  # Limit to top 5 positions
                stock_rec = await self._generate_individual_stock_recommendation(
    ...
    async def calculate_technical_indicators(
        self, ts_code: str, period: int = 60
    ) -> dict[str, Any]:
        """计算技术指标."""
        try:
            ...
            indicators["ma60"] = (
                price_data["close"].rolling(60).mean().iloc[-1]
            ...
            if "volume" in price_data.columns:
                indicators["volume_ma5"] = (
                    price_data["volume"].rolling(5).mean().iloc[-1]
                )
                indicators["volume_ratio"] = (
                    price_data["volume"].iloc[-1] / indicators["volume_ma5"]
                )
            ...
            indicators["price_change_pct"] = (
                indicators["price_change"] / price_data["close"].iloc[-2]
            ) * 100
    ...
    def _calculate_performance_metrics(
        ...
        return {
            ...
            "max_drawdown": self._calculate_max_drawdown(df["total_profit"])
            if len(df) > 1
            else 0,
    ...
    def _analyze_sector_allocation(
        self, sector_distribution: dict[str, float]
    ) -> dict[str, Any]:
        """Analyze sector allocation."""
        ...
        if total_allocation > 0:
            normalized_weights = [
                w / total_allocation for w in sector_distribution.values()
            ]
    ...
    def _assess_liquidity_risk(self, positions: list) -> dict[str, Any]:
        ...
        return {"liquidity_score": 0.8, "risk_level": "低", "recommendations": []}
    ...
    async def _assess_correlation_risk(self, positions: list) -> dict[str, Any]:
        ...
        return {"average_correlation": 0.3, "risk_level": "低", "recommendations": []}
    ...
    async def _get_price_data(self, ts_code: str, period: int) -> pd.DataFrame | None:
        """Get historical price data."""
    ...
    def _generate_stock_recommendation(self, analysis_result: dict) -> dict[str, Any]:
        ...
        if "fundamental_analysis" in analysis_result:
            fund_score = analysis_result["fundamental_analysis"].get(
                "overall_score", 50
            )
    ...
    async def _generate_portfolio_recommendations(
        self, summary, market_condition: str
    ) -> list[dict[str, Any]]:
    ...
    async def _generate_individual_stock_recommendation(
        self, position, market_condition: str
    ) -> dict[str, Any]:
    ...
    def _analyze_current_allocation(self, positions: list, summary) -> dict[str, Any]:
        ...
        allocation = {
            ...
            "risk_metrics": {"concentration": 0.0, "diversification": 0.0},
        }
    ...
    def _generate_recommended_allocation(
        self, current_allocation: dict, target_risk: str
    ) -> dict[str, Any]:
    ...
    async def analyze_portfolio_toplist(self) -> dict[str, Any]:
        ...
        try:
            ...
            for position in positions:
                ...
                # 获取该股票的龙虎榜历史
                history = await self.toplist_service.get_stock_top_list_history(
                    ts_code, 5
                )

                if history:
                    # 计算席位集中度
                    concentration = await self.toplist_analysis_service.calculate_seat_concentration(
                        ts_code, 5
                    )
                    ...
                    position_analysis = {
                        ...
                        "latest_appearance": history[0]["trade_date"]
                        if history
                        else None,
                        "concentration_index": concentration.get(
                            "concentration_index", 0
                        ),
                        "institution_dominance": concentration.get(
                            "institution_dominance", 0
                        ),
                        "recent_net_flow": sum(
                            item.get("net_amount", 0) for item in history
                        ),
            ...
            # 生成投资建议
            toplist_analysis["investment_suggestions"] = (
                self._generate_toplist_suggestions(
            ...
            # 整体资金流向分析
            total_net_flow = sum(
                pos["recent_net_flow"] for pos in toplist_analysis["on_list_positions"]
            )
            avg_concentration = (
                np.mean(
                    [
                        pos["concentration_index"]
                        for pos in toplist_analysis["on_list_positions"]
                    ]
                )
                if toplist_analysis["on_list_positions"]
                else 0
            )
        ...
        try:
            ...
            for position in positions:
                ...
                if ts_code in toplist_codes:
                    # 找到对应的龙虎榜数据
                    toplist_data = next(
                        (item for item in today_toplist if item["ts_code"] == ts_code),
                        None,
                    )
                    if toplist_data:
            ...
            return {
                ...
                "data": {
                    ...
                    "toplist_ratio": on_toplist_count / len(status_results)
                    if status_results
                    else 0,
                },
    ...
    async def analyze_position_capital_flow(self, days: int = 5) -> dict[str, Any]:
        ...
        try:
            ...
            for position in positions:
                ...
                try:
                    ...
                    if history:
                        ...
                        avg_pct_chg = np.mean(
                            [item.get("pct_chg", 0) for item in history]
                        )
                        ...
                        position_flow = {
                            ...
                            "risk_assessment": self._assess_flow_risk(
                                net_flow, concentration, avg_pct_chg
                            ),
                        }
            ...
            # 按净流入排序
            flow_analysis["position_flows"].sort(
                key=lambda x: x["net_flow"], reverse=True
            )
    ...
    def _assess_flow_risk(
        self, net_flow: float, concentration: dict, avg_pct_chg: float
    ) -> str:
        """评估资金流向风险"""
        risk_factors = []
    ...
    def _generate_toplist_suggestions(self, positions: list[dict]) -> list[str]:
        ...
        # 资金流出建议
        outflow_positions = [
            pos for pos in positions if pos["recent_net_flow"] < -50000
        ]
        if outflow_positions:
            suggestions.append(
                f"{len(outflow_positions)}只股票存在明显资金流出，建议密切关注"
            )

        # 席位集中度建议
        high_concentration = [
            pos for pos in positions if pos["concentration_index"] > 0.6
        ]
        if high_concentration:
            suggestions.append(
                f"{len(high_concentration)}只股票席位集中度较高，注意流动性风险"
            )

        # 机构主导建议
        institution_dominated = [
            pos for pos in positions if pos["institution_dominance"] > 0.7
        ]
        if institution_dominated:
            suggestions.append(
                f"{len(institution_dominated)}只股票机构主导明显，可关注后续动向"
            )
