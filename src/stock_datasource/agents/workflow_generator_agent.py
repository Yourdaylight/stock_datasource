...
from stock_datasource.models.workflow import ToolInfo
...
请生成完整的工作流JSON配置。"""
        ...
        async for event in self.execute_stream(user_message, context):
            ...
            if event_type == "thinking":
                yield {"type": "thinking", "content": event.get("status", "思考中...")}

            elif event_type == "content":
                content = event.get("content", "")
                full_response += content
                yield {"type": "generating", "content": content}
                ...
                try:
                    ...
                    if workflow_config:
                        yield {"type": "workflow", "workflow": workflow_config}
                    else:
                        yield {"type": "error", "error": "无法解析生成的工作流配置"}
                except Exception as e:
                    logger.error(f"Failed to parse workflow config: {e}")
                    yield {"type": "error", "error": f"解析配置失败: {e!s}"}
    ...
    def _parse_workflow_config(self, response: str) -> dict[str, Any] | None:
        """解析AI生成的工作流配置。"""
...
# 单例
_generator_agent: WorkflowGeneratorAgent | None = None
