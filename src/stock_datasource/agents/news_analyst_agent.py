...
- get_stock_signal_summary: 获取股票消息面信号聚合评分
...
def get_stock_signal_summary(
    stock_code: str, days: int = 7
) -> dict[str, Any]:
    """获取股票的消息面信号摘要(用于信号聚合)
    ...
    """
    try:
        from stock_datasource.modules.signal_aggregator.schemas import NewsSignalInput
        from stock_datasource.modules.signal_aggregator.service import get_signal_aggregator

        aggregator = get_signal_aggregator()

        # 使用同步方式调用异步方法
        result = _run_async_safely(aggregator.aggregate_for_stock(stock_code))

        if result is None:
            return {"message": f"无法为 {stock_code} 生成信号摘要", "stock_code": stock_code}
        ...
        logger.error(f"get_stock_signal_summary error: {e}", exc_info=True)
        return {"message": f"获取信号摘要失败: {e!s}", "stock_code": stock_code}
