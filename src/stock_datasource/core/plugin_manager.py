...
class DependencyNotSatisfiedError(Exception):
    ...
    def __init__(
        ...
        plugin_name: str,
        missing: list[str],
        missing_data: dict[str, str] | None = None,
    ):
...
class PluginManager:
                        ...
                        try:
                            ...
                            for attr_name in dir(module):
                                ...
                                if (
                                    ...
                                    discovered += 1
                                    self.logger.debug(
                                        f"Discovered plugin after retry: {plugin_instance.name}"
                                    )
                            continue
                        except Exception as retry_error:
                            failed += 1
                            self.logger.error(
                                f"Failed to load plugin module {name} after retry: {retry_error}"
                            )
                            continue
                ...
                info.append(
                    {
                        ...
                        "description": getattr(
                            plugin, "description", "Error loading plugin info"
                        ),
                        "rate_limit": 60,
                        "dependencies": [],
    ...
    def check_dependencies(self, plugin_name: str) -> DependencyCheckResult:
        ...
        plugin = self.get_plugin(plugin_name)
        if not plugin:
            return DependencyCheckResult(satisfied=False, missing_plugins=[plugin_name])
        ...
        if not dependencies:
            return DependencyCheckResult(
                satisfied=True, optional_dependencies=optional_deps
            )
        ...
        for dep_name in dependencies:
            ...
            try:
                ...
                self.logger.warning(
                    f"Failed to check data for dependency {dep_name}: {e}"
                )
        ...
        if dep_result.has_issues:
            if not auto_run_deps:
                raise DependencyNotSatisfiedError(
                    plugin_name, dep_result.missing_plugins, dep_result.missing_data
                )
            ...
            if dep_result.missing_plugins:
                raise DependencyNotSatisfiedError(
                    plugin_name, dep_result.missing_plugins, {}
                )
            ...
            for dep_name in dep_result.missing_data.keys():
                ...
                try:
                    ...
                    raise DependencyNotSatisfiedError(
                        plugin_name, [], {dep_name: f"Execution failed: {e!s}"}
                    )
            ...
            for opt_dep_name in dep_result.optional_dependencies:
                ...
                if opt_plugin and opt_plugin.is_enabled():
                    ...
                    try:
                        ...
                        self.logger.warning(
                            f"Optional dependency {opt_dep_name} failed: {e}"
                        )
                        optional_results[opt_dep_name] = {
                            "status": "failed",
                            "error": str(e),
                        }
    ...
    def get_dependency_graph(self) -> dict[str, list[str]]:
        ...
        for plugin_name, plugin in self.plugins.items():
            try:
                ...
                self.logger.warning(
                    f"Failed to get dependencies for plugin {plugin_name}: {e}"
                )
                graph[plugin_name] = []
        return graph
    ...
    def get_reverse_dependencies(self, plugin_name: str) -> list[str]:
        ...
        for name, plugin in self.plugins.items():
            try:
                ...
                self.logger.warning(
                    f"Failed to check dependencies for plugin {name}: {e}"
                )
        return dependents
    ...
    def get_filtered_plugins(
        self, category: PluginCategory | None = None, role: PluginRole | None = None
    ) -> list[BasePlugin]:
