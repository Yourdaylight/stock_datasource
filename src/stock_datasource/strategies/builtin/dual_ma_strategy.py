...
class DualMAStrategy(BaseStrategy):
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        if ma_type == "SMA":
            ...
            df["ma_fast"] = (
                df["close"]
                .rolling(fast_period)
                .apply(lambda x: np.average(x, weights=weights_fast), raw=True)
            )
            df["ma_slow"] = (
                df["close"]
                .rolling(slow_period)
                .apply(lambda x: np.average(x, weights=weights_slow), raw=True)
            )
            if self.params.get("use_trend_filter", True):
                weights_trend = np.arange(1, trend_filter_period + 1)
                df["ma_trend"] = (
                    df["close"]
                    .rolling(trend_filter_period)
                    .apply(lambda x: np.average(x, weights=weights_trend), raw=True)
                )
        else:  # EMA
    ...
    def generate_signals(self, data: pd.DataFrame) -> list[TradingSignal]:
        ...
        for idx, row in df.iterrows():
            if (
                pd.isna(row["dual_ma_signal"])
                or row["dual_ma_signal"] == 0
                or pd.isna(row["ma_fast"])
                or pd.isna(row["ma_slow"])
            ):
                continue
    ...
    def _get_buy_reason(self, row: pd.Series) -> str:
        ...
        reason = (
            f"快线({self.params['fast_period']})上穿慢线({self.params['slow_period']}) "
        )
        reason += (
            f"(快线: {fast_ma:.2f}, 慢线: {slow_ma:.2f}, 分离度: {separation:.2%})"
        )
    ...
    def _get_sell_reason(self, row: pd.Series) -> str:
        ...
        reason = (
            f"快线({self.params['fast_period']})下穿慢线({self.params['slow_period']}) "
        )
        reason += (
            f"(快线: {fast_ma:.2f}, 慢线: {slow_ma:.2f}, 分离度: {separation:.2%})"
        )
    ...
    def _calculate_signal_confidence(self, row: pd.Series, signal_type: str) -> float:
        ...
        if self.params.get("use_trend_filter", True):
            ...
            if (signal_type == "buy" and price > trend_ma) or (
                signal_type == "sell" and price < trend_ma
            ):
                trend_bonus = 0.2

        # 综合置信度
        confidence = (
            separation_confidence * 0.4 + slope_confidence * 0.4 + trend_bonus + 0.3
        )
