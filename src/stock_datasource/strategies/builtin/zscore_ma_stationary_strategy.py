...
class ZScoreMAStationaryStrategy(BaseStrategy):
    ...
    def calculate_indicators(self, data: pd.DataFrame) -> pd.DataFrame:
        ...
        df["ma_spread_zscore"] = (df["ma_spread"] - df["ma_spread_mean"]) / df[
            "ma_spread_std"
        ]
        ...
        df["ma_rs_zscore"] = (df["ma_relative_strength"] - df["ma_rs_mean"]) / df[
            "ma_rs_std"
        ]
        ...
        df["price_ma_zscore"] = (
            df["price_ma_deviation"]
            - df["price_ma_deviation"].rolling(zscore_lookback).mean()
    ...
    def generate_signals(self, data: pd.DataFrame) -> list[TradingSignal]:
        ...
        for idx, row in df.iterrows():
            # 跳过数据不足的行
            if (
                pd.isna(row["ma_spread_zscore"])
                or pd.isna(row["ma_rs_zscore"])
                or pd.isna(row["price_ma_zscore"])
            ):
                continue
            ...
            signal_strength = self._calculate_signal_strength(
                row, zscore_threshold, rs_threshold
            )
    ...
    def _calculate_signal_strength(
        self, row: pd.Series, zscore_threshold: float, rs_threshold: float
    ) -> float:
        """计算综合信号强度"""
    ...
    def _apply_filters(
        self, row: pd.Series, volume_filter: bool, momentum_filter: bool
    ) -> bool:
        """应用过滤条件"""
        ...
        if momentum_filter:
            # 买入信号需要正动量支持
            if row["ma_spread_zscore"] > 0 and row["momentum_5"] < -0.02:  # 5日动量为负
                return False

            # 卖出信号需要负动量支持
            if row["ma_spread_zscore"] < 0 and row["momentum_5"] > 0.02:  # 5日动量为正
                return False
    ...
    def _calculate_confidence(self, row: pd.Series, signal_strength: float) -> float:
        ...
        # 成交量加成
        volume_boost = (
            min((row["volume_ratio"] - 1.0) * 0.2, 0.2)
        ...
        # 综合置信度
        total_confidence = (
            base_confidence + volume_boost + momentum_boost + zscore_boost
        )
...
- 严格的参数验证机制
    ...
    def get_risk_metrics(self, data: pd.DataFrame) -> dict[str, float]:
        ...
        signal_frequency = (
            df["ma_spread_zscore"].abs() > self.params.get("zscore_threshold", 1.5)
        ).mean()
