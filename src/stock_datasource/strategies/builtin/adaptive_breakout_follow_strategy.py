...
class AdaptiveBreakoutFollowStrategy(BaseStrategy):
    ...
    def _calculate_breakout_indicators(
        ...
        df["breakout_strength_up"] = (df["close"] - df["breakout_high"].shift(1)) / df[
            "atr"
        ]
        df["breakout_strength_down"] = (df["breakout_low"].shift(1) - df["close"]) / df[
            "atr"
        ]

        # 通道收缩检测（提高突破质量）
        df["channel_squeeze"] = (
            df["channel_width"] < df["channel_width"].rolling(period).mean() * 0.8
        )

        return df

    def _calculate_trend_following_indicators(
        self, df: pd.DataFrame, fast: int, slow: int
    ) -> pd.DataFrame:
        """计算趋势跟踪指标（优化滞后性）"""
        # 快速响应的EMA
    ...
    def _calculate_market_regime_indicators(
        ...
        df["volatility_percentile"] = (
            df["volatility"].rolling(lookback * 2).rank(pct=True)
        )
        ...
        df.loc[df["is_trending"] & df["is_high_volatility"], "market_regime"] = (
            "trending_volatile"
        )
        df.loc[df["is_trending"] & df["is_low_volatility"], "market_regime"] = (
            "trending_stable"
        )
        df.loc[~df["is_trending"] & df["is_high_volatility"], "market_regime"] = (
            "ranging_volatile"
    ...
    def _calculate_volume_indicators(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        df["price_volume_sync"] = (price_up & df["volume_surge"]) | (
            ~price_up & ~df["volume_surge"]
        )
    ...
    def _select_strategy(self, row: pd.Series) -> str:
        ...
        if market_regime == "trending_volatile":
            ...
            # 震荡高波动：谨慎使用突破策略
            return (
                "breakout" if row["channel_squeeze"] and row["volume_surge"] else None
            )
            ...
            return (
                "following"
                if row["trend_up_confirmed"] or row["trend_down_confirmed"]
                else None
            )
            ...
            breakout_strength = max(
                row.get("breakout_strength_up", 0), row.get("breakout_strength_down", 0)
            )
    ...
    def _generate_breakout_signal(
        self, row: pd.Series, timestamp, symbol: str, price: float
    ) -> TradingSignal:
        """生成突破策略信号"""
        ...
        # 向上突破
        if (
            row["price_above_channel"]
            and row["breakout_strength_up"] >= min_strength
            and row["volume_surge"]
            and row["obv_trend"]
        ):
        ...
        # 向下突破
            ...
            row["price_below_channel"]
            and row["breakout_strength_down"] >= min_strength
            and row["volume_surge"]
            and not row["obv_trend"]
        ):
    ...
    def _generate_following_signal(
        self, row: pd.Series, timestamp, symbol: str, price: float
    ) -> TradingSignal:
        """生成趋势跟踪信号"""
    ...
    def _calculate_breakout_confidence(self, row: pd.Series, action: str) -> float:
        ...
        regime_boost = (
            0.1
            if row["market_regime"] in ["trending_volatile", "trending_stable"]
            else 0
        )
    ...
    def _calculate_following_confidence(self, row: pd.Series, action: str) -> float:
        ...
        total_confidence = (
            base_confidence + adx_boost + macd_boost + ma_boost + sync_boost
        )
        return min(total_confidence, 1.0)
...
- 程序化交易系统
    ...
    def get_strategy_metrics(self, data: pd.DataFrame) -> dict[str, Any]:
        ...
        return {
            "breakout_signal_ratio": breakout_signals / total_signals
            if total_signals > 0
            else 0,
            "following_signal_ratio": following_signals / total_signals
            if total_signals > 0
            else 0,
            "avg_volatility": df["volatility"].mean(),
            "avg_adx": df["adx"].mean(),
            "trending_market_ratio": (
                df["market_regime"].str.contains("trending")
            ).mean(),
            "avg_volume_ratio": df["volume_ratio"].mean(),
            "strategy_adaptability": len(df["market_regime"].unique())
            / 5,  # 市场环境多样性
        }
