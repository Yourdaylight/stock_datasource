...
class OptimizationAlgorithm:
    ...
    def _evaluate_objectives(self, result: Any) -> float:
        ...
        for objective in self.config.objectives:
            ...
            elif hasattr(result, "performance_metrics") and hasattr(
                result.performance_metrics, objective.name
            ):
...
class GridSearchOptimizer(OptimizationAlgorithm):
    ...
    async def _parallel_evaluate(
        self, param_grid: list[dict[str, Any]], objective_function: Callable
...
class StrategyOptimizer:
    ...
    async def optimize(
    ...
    ) -> tuple[BaseStrategy, OptimizationResult]:
        """
        优化策略参数
        ...
        try:
            ...
            optimal_params = await algorithm.optimize(
                param_space, objective_function, config.constraints
            )
            ...
            if optimal_params:
                ...
                for objective in config.objectives:
                    if hasattr(final_result, objective.name):
                        objective_values[objective.name] = getattr(
                            final_result, objective.name
                        )
                    elif hasattr(final_result, "performance_metrics"):
                        if hasattr(final_result.performance_metrics, objective.name):
                            objective_values[objective.name] = getattr(
                                final_result.performance_metrics, objective.name
                            )
    ...
    def create_optimization_config(
        ...
        for obj_name in objectives:
            ...
            objective_objects.append(
                OptimizationObjective(name=obj_name, type=obj_type, weight=1.0)
            )
