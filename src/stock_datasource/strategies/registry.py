...
from .base import AIGeneratedStrategy, BaseStrategy, StrategyCategory, StrategyMetadata
...
class StrategyRegistry:
    ...
    def register_strategy(
        ...
        try:
            ...
            if actual_id in self._strategies and not overwrite:
                logger.warning(
                    f"Strategy ID {actual_id} already exists, use overwrite=True to replace"
                )
                return False
    ...
    def register_builtin_strategy(self, strategy_class: type[BaseStrategy]) -> bool:
        ...
        try:
            ...
            if metadata.id in self._strategies:
                logger.warning(
                    f"Strategy ID {metadata.id} already exists, skipping registration"
                )
                return False
    ...
    def register_ai_strategy(self, strategy: AIGeneratedStrategy) -> bool:
        ...
        try:
            ...
            logger.info(
                f"Registered AI strategy: {strategy.metadata.name} ({strategy_id})"
            )
            return True
    ...
    def get_strategy_class(self, strategy_id: str) -> type[BaseStrategy] | None:
        """
        获取策略类
    ...
    def get_strategy(
        self, strategy_id: str, params: dict[str, Any] = None
    ) -> BaseStrategy | None:
        """
        获取策略实例
        ...
        try:
            ...
            if strategy_id in self._ai_strategies:
                ...
                if params:
                    # 创建新实例并应用参数
                    new_strategy = ai_strategy.__class__(
                        params, ai_strategy.generation_info
                    )
                    return new_strategy
                return ai_strategy
