...
class StrategyTemplate:
    ...
    def __init__(
        self, name: str, code_template: str, parameters: list[ParameterSchema]
    ):
        self.name = name
...
class LLMAdapter:
    ...
    async def generate_strategy_code(
        self, intent: StrategyIntent, template: StrategyTemplate
    ) -> str:
        """生成策略代码"""
...
class StrategyKnowledgeBase:
    ...
    def find_template(self, intent: StrategyIntent) -> StrategyTemplate | None:
        """根据意图查找最匹配的模板"""
...
class AIStrategyGenerator:
    ...
    async def generate_from_description(
        self, description: str, user_profile: dict[str, Any] | None = None
    ) -> AIGeneratedStrategy:
        """
        基于自然语言描述生成策略
        ...
        try:
            ...
            # 3. 生成策略代码
            strategy_code = await self.llm_adapter.generate_strategy_code(
                intent, template
            )

            # 4. 验证和创建策略
            strategy = await self._create_strategy_from_code(
                strategy_code, intent, description
            )
    ...
    async def _create_strategy_from_code(
        self, code: str, intent: StrategyIntent, original_description: str
    ) -> AIGeneratedStrategy:
        """从代码创建策略实例"""
    ...
    def _create_simple_strategy(
        self, intent: StrategyIntent, strategy_id: str, generation_info: dict[str, Any]
    ) -> AIGeneratedStrategy:
        """创建简化的策略实例"""
        ...
        class SimpleAIStrategy(AIGeneratedStrategy):
            ...
            def get_parameter_schema(self):
                ...
                if self.intent_data.strategy_type == "moving_average":
                    return [
                        ParameterSchema(
                            "short_period", "int", 5, 1, 50, "短期均线周期"
                        ),
                        ParameterSchema(
                            "long_period", "int", 20, 2, 100, "长期均线周期"
                        ),
                    ]
                    ...
                    return [
                        ParameterSchema(
                            "threshold", "float", 0.02, 0.01, 0.1, "信号阈值"
                        )
                    ]
            ...
            def _generate_ma_signals(self, data):
                ...
                for i in range(1, len(df)):
                    if pd.isna(df["ma_short"].iloc[i]) or pd.isna(
                        df["ma_long"].iloc[i]
                    ):
                        continue
                    ...
                    if (
                        ...
                        timestamp = pd.to_datetime(
                            df.iloc[i].get("timestamp", datetime.now())
                        )
                        signal = TradingSignal(
                            timestamp=timestamp,
                            symbol=symbol,
            ...
            def _generate_default_signals(self, data):
                ...
                for i in range(1, len(df)):
                    price_change = (df["close"].iloc[i] - df["close"].iloc[i - 1]) / df[
                        "close"
                    ].iloc[i - 1]
...
{chr(10).join(f"- {warning}" for warning in strategy.get_risk_warnings())}
    ...
    async def optimize_strategy(
        self, strategy: AIGeneratedStrategy, backtest_result: Any
    ) -> AIGeneratedStrategy:
        """优化AI策略"""
        # 获取改进建议
