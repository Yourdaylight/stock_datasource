    ...
    def extract_data(self, **kwargs) -> pd.DataFrame:
        ...
        if not ts_code:
            ...
            for idx, row in stocks_df.iterrows():
                ...
                try:
                    self.logger.info(
                        f"Extracting express data for {stock_code} ({idx + 1}/{len(stocks_df)})"
                    )
                    ...
                    self.logger.warning(
                        f"Failed to extract express for {stock_code}: {e}"
                    )
                    continue
            ...
            self.logger.info(
                f"Extracted {len(combined_data)} express records from {len(all_data)} stocks"
            )
