"""TuShare sz_daily_info data plugin implementation."""
...
class TuShareSzDailyInfoPlugin(BasePlugin):
    """TuShare sz_daily_info data plugin (深圳市场每日概况)."""
    
    @property
    def name(self) -> str:
        return "tushare_sz_daily_info"
    ...
    def description(self) -> str:
        return "TuShare SZ market daily info (深圳市场每日交易概况)"
    ...
    def extract_data(self, **kwargs) -> pd.DataFrame:
        ...
        if start_date and end_date:
            data = extractor.extract_by_date_range(start_date, end_date, ts_code)
        elif trade_date:
