...
class TuShareBalancesheetVipPlugin(BasePlugin):
    ...
    def extract_data(self, **kwargs) -> pd.DataFrame:
        ...
        self.logger.info(
            f"Extracting balance sheet VIP data (period={period}, report_type={report_type})"
        )
    ...
    def validate_data(self, data: pd.DataFrame) -> bool:
        ...
        self.logger.info(
            f"Balance sheet VIP data validation passed for {len(data)} records"
        )
        return True
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(
        description="TuShare Balance Sheet VIP Data Plugin"
    )
    parser.add_argument(
