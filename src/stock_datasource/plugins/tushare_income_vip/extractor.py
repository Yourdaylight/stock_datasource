"""TuShare income statement VIP data extractor."""
...
class IncomeVipExtractor:
    """Extractor for TuShare income statement VIP data (batch mode)."""
    ...
    def _call_api(self, **kwargs) -> pd.DataFrame:
        """Call TuShare income_vip API with rate limiting and retry."""
        self._rate_limit()
        
        try:
            result = self.pro.income_vip(**kwargs)
    ...
    def extract(self, ts_code: Optional[str] = None, ann_date: Optional[str] = None,
        ...
        """Extract income statement VIP data.
