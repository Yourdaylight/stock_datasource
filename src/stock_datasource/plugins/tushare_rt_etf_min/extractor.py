    ...
    def _call_api(self, ts_code: str, freq: str) -> pd.DataFrame:
        ...
        try:
            ...
            if result is None or result.empty:
                logger.warning(
                    f"API returned empty data for ETF ts_code={ts_code}, freq={freq}"
                )
                return pd.DataFrame()

            logger.info(
                f"API call successful: ETF ts_code={ts_code}, freq={freq}, records={len(result)}"
            )
            return result

        except Exception as e:
            logger.error(
                f"API call failed: ETF ts_code={ts_code}, freq={freq}, error={e}"
            )
            raise
