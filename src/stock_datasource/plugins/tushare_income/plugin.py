...
class TuShareIncomePlugin(BasePlugin):
    ...
    def extract_data(self, **kwargs) -> pd.DataFrame:
        ...
        if not ts_code:
            ...
            self.logger.info("Extracting income statement data for all stocks (batch mode)")
            ...
            for idx, row in stocks_df.iterrows():
                ...
                try:
                    self.logger.info(f"Extracting income statement data for {stock_code} ({idx+1}/{len(stocks_df)})")
                    ...
                    self.logger.warning(f"Failed to extract income statement for {stock_code}: {e}")
                    continue
            ...
            self.logger.info(f"Extracted {len(combined_data)} income statement records from {len(all_data)} stocks")
    ...
    def validate_data(self, data: pd.DataFrame) -> bool:
        ...
        self.logger.info(f"Income statement data validation passed for {len(data)} records")
        return True
...
if __name__ == "__main__":
    ...
    parser.add_argument("--start-date", help="Announcement start date in YYYYMMDD format")
