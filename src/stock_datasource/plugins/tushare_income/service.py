...
class TuShareIncomeService(BaseService):
    ...
    @query_method(
        ...
        params=[
            ...
            QueryParam(
                name="report_type",
                type="str",
                description="Report type: 1=合并报表, 2=单季合并",
                required=False,
            ),
        ],
    )
    def get_income_statement(
        self, code: str, periods: int | None = 8, report_type: str | None = "1"
    ) -> list[dict[str, Any]]:
    ...
    def get_profitability_metrics(
        ...
        for _, row in df.iterrows():
            ...
            if rev and rev != 0:
                metric["gross_margin"] = (
                    (
                        (rev - (metric["oper_cost"] or metric["total_cogs"] or 0))
                        / rev
                        * 100
                    )
                    if (metric["oper_cost"] or metric["total_cogs"])
                    else None
                )
                metric["operating_margin"] = (
                    (metric["operate_profit"] / rev * 100)
                    if metric["operate_profit"]
                    else None
                )
                metric["net_margin"] = (
                    (metric["net_income"] / rev * 100) if metric["net_income"] else None
                )
                # Expense ratios (as % of revenue)
                metric["sell_exp_ratio"] = (
                    (metric["sell_exp"] / rev * 100) if metric["sell_exp"] else None
                )
                metric["admin_exp_ratio"] = (
                    (metric["admin_exp"] / rev * 100) if metric["admin_exp"] else None
                )
                metric["fin_exp_ratio"] = (
                    (metric["fin_exp"] / rev * 100) if metric["fin_exp"] else None
                )
                metric["rd_exp_ratio"] = (
                    (metric["rd_exp"] / rev * 100) if metric["rd_exp"] else None
                )

            metrics.append(metric)

        return {"code": code, "periods": len(metrics), "metrics": metrics}
    ...
    def get_quarterly_income(
        self, code: str, quarters: int | None = 8
    ) -> list[dict[str, Any]]:
