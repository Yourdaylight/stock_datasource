...
class TuShareStockSTPlugin(BasePlugin):
    ...
    def extract_data(self, **kwargs) -> pd.DataFrame:
        ...
        self.logger.info(f"Extracting stock ST data: trade_date={trade_date}, ts_code={ts_code}")
    ...
    def validate_data(self, data: pd.DataFrame) -> bool:
        ...
        if null_ts_codes > 0 or null_names > 0 or null_dates > 0:
            self.logger.error(f"Found null values: ts_code={null_ts_codes}, name={null_names}, trade_date={null_dates}")
            return False
