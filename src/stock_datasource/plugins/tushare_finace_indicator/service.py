...
class TuShareFinaceIndicatorService(BaseService):
    ...
    def _sanitize_limit(
        self, limit: int | None, default: int = 1000, max_limit: int = 10000
    ) -> int:
    ...
    @query_method(
        ...
        params=[
            QueryParam(
                name="code",
                type="str",
                description="Stock code (e.g., 002579.SZ)",
                required=False,
            ),
            QueryParam(
                name="start_date",
    ...
    @query_method(
        ...
        params=[
            ...
            QueryParam(
                name="periods",
                type="int",
                description="Number of latest periods to return",
                required=False,
            ),
        ],
    )
    def get_financial_summary(
        ...
        if len(latest_data) >= 2:
            ...
            try:
                curr_revenue = (
                    float(current.get("total_revenue"))
                    if current.get("total_revenue") is not None
                    else None
                )
                prev_revenue = (
                    float(previous.get("total_revenue"))
                    if previous.get("total_revenue") is not None
                    else None
                )
                if (
                    curr_revenue is not None
            ...
            try:
                curr_profit = (
                    float(current.get("net_profit"))
                    if current.get("net_profit") is not None
                    else None
                )
                prev_profit = (
                    float(previous.get("net_profit"))
                    if previous.get("net_profit") is not None
                    else None
                )
                if (
                    curr_profit is not None
                    and prev_profit is not None
                    and prev_profit != 0
                ):
                    growth["profit_growth"] = (
                        (curr_profit - prev_profit) / prev_profit * 100
                    )
            except (ValueError, TypeError):
                pass
    ...
    @query_method(
        ...
        params=[
            ...
            QueryParam(
                name="periods",
                type="int",
                description="Number of periods for trend analysis",
                required=False,
            ),
        ],
    )
    ...
    @query_method(
        ...
        params=[
            QueryParam(
                name="code", type="str", description="Target stock code", required=True
            ),
            QueryParam(
                name="end_date",
                type="str",
            ...
            QueryParam(
                name="industry_limit",
                type="int",
                description="Number of peer companies to compare",
                required=False,
            ),
        ],
    )
    def get_peer_comparison(
        self, code: str, end_date: str, industry_limit: int | None = 20
    ) -> dict[str, Any]:
        ...
        LIMIT %(limit)s
        """
        peers_params = {"end_date": end_date, "code": code, "limit": str(limit_value)}
                ...
                if not peer_values.empty and target_value is not None:
                    comparison[metric] = {
                        ...
                        "percentile_rank": float(
                            (peer_values < target_value).sum() / len(peer_values) * 100
                        ),
                    }
        ...
        return {
            ...
            "peer_companies": peers_df[["ts_code", "roe", "roa"]].to_dict("records")[
                :10
            ],  # Top 10 peers
        }
    ...
    def analyze_financial_health(
                ...
                if "ROE" in w:
                    ...
                    if roe_v is not None and roe_v < 0:
                        recommendations.append(
                            f"ROE为{roe_v:.1f}%，公司处于亏损，重点关注扭亏为盈的时间表和战略规划"
                        )
                    elif roe_v is not None:
                        recommendations.append(
                            f"ROE仅{roe_v:.1f}%，建议关注管理层提升资本回报率的具体措施"
                        )
                    break
            for w in weaknesses:
                if "负债率" in w:
                    ...
                    if dta_v is not None:
                        recommendations.append(
                            f"资产负债率达{dta_v:.1f}%，建议关注偿债压力和融资成本变化"
                        )
                    break
            for w in weaknesses:
                if "下滑" in w:
                    ...
                    if rg_v is not None:
                        recommendations.append(
                            f"营收增速{rg_v:.1f}%，建议关注行业竞争格局和公司新增长点"
                        )
                    else:
                        recommendations.append(
                            "业绩出现下滑，建议关注是行业周期性波动还是公司竞争力下降"
                        )
                    break
            for w in weaknesses:
                if "净利率" in w:
                    recommendations.append(
                        "净利率偏低可能反映费用管控不力或竞争加剧，关注公司降本增效举措"
                    )
                    break
            for w in weaknesses:
                if "流动比率" in w:
                    recommendations.append(
                        "短期偿债能力偏弱，关注公司现金流状况和短期债务到期安排"
                    )
                    break
        ...
        if not recommendations:
            if health_score >= 75:
                recommendations.append(
                    "财务基本面优秀，可重点关注估值是否合理以及行业景气度"
                )
            elif health_score >= 60:
                recommendations.append(
                    "财务状况总体良好，建议结合行业趋势和公司战略综合判断"
                )
            elif strengths:
                recommendations.append(
                    "公司有一定亮点但也存在不足，建议持续跟踪后续财报变化"
                )
            else:
                recommendations.append(
                    "基本面数据较为平淡，建议深入分析公司战略定位和行业前景"
                )
    ...
    @query_method(
        ...
        params=[
            ...
            QueryParam(
                name="max_roe",
                type="float",
                description="Maximum ROE filter",
                required=False,
            ),
            QueryParam(
                name="limit",
    ...
    def get_indicators_by_date(
        ...
        if min_roe is not None and (
            not isinstance(min_roe, (int, float)) or min_roe < -100 or min_roe > 1000
        ):
            raise ValueError("min_roe must be a number between -100 and 1000")

        if max_roe is not None and (
            not isinstance(max_roe, (int, float)) or max_roe < -100 or max_roe > 1000
        ):
    ...
    @query_method(
        ...
        params=[
            QueryParam(
                name="code", type="str", description="Stock code", required=True
            ),
            QueryParam(
                name="periods",
                type="int",
    ...
    def get_roe_trend(self, code: str, periods: int | None = 8) -> list[dict[str, Any]]:
