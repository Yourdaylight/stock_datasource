        ...
        for _, row in df.iterrows():
            ...
            if row["total_cur_liab"] and row["total_cur_liab"] != 0:
                metric["current_ratio"] = (
                    row["total_cur_assets"] / row["total_cur_liab"]
                    if row["total_cur_assets"]
                    else None
                )
                quick_assets = (row["total_cur_assets"] or 0) - (
                    row["inventories"] or 0
                )
                metric["quick_ratio"] = quick_assets / row["total_cur_liab"]

            if row["total_assets"] and row["total_assets"] != 0:
                metric["debt_to_assets"] = (
                    (row["total_liab"] / row["total_assets"] * 100)
                    if row["total_liab"]
                    else None
                )

            # Working capital
            metric["working_capital"] = (row["total_cur_assets"] or 0) - (
                row["total_cur_liab"] or 0
            )
