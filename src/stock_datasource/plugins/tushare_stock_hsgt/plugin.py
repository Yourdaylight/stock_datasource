...
class TuShareStockHSGTPlugin(BasePlugin):
    ...
    def extract_data(self, **kwargs) -> pd.DataFrame:
        ...
        if not hsgt_type:
            ...
            self.logger.info(
                f"Extracting stock HSGT data: type={hsgt_type}, trade_date={trade_date}"
            )
            data = extractor.extract(
                hsgt_type=hsgt_type,
                trade_date=trade_date,
    ...
    def validate_data(self, data: pd.DataFrame) -> bool:
        ...
        if null_ts_codes > 0 or null_dates > 0 or null_types > 0:
            self.logger.error(
                f"Found null values: ts_code={null_ts_codes}, trade_date={null_dates}, type={null_types}"
            )
            return False
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--type",
        choices=["HK_SZ", "SZ_HK", "HK_SH", "SH_HK"],
