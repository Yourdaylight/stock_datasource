"""Query service for TuShare cash flow VIP data."""
...
class TuShareCashflowVipService(BaseService):
    """Query service for TuShare cash flow VIP data.

    Note: VIP data is stored in the same table as base cashflow (ods_cash_flow).
    This service provides the same query methods as the base cashflow service.
    """

    def __init__(self):
        super().__init__("tushare_cashflow_vip")
    ...
    @query_method(
        description="Query cash flow by period (batch)",
        params=[
            QueryParam(
                name="period",
                type="str",
            ...
            QueryParam(
                name="report_type",
                type="str",
                description="Report type: 1=合并报表",
                required=False,
            ),
            QueryParam(
                name="limit",
    ...
    def get_cashflow_by_period(
        self, period: str, report_type: str | None = "1", limit: int | None = 1000
    ) -> list[dict[str, Any]]:
        """Query cash flow data for a period (all stocks)."""
    ...
    @query_method(
        description="Get cash flow summary by period",
        params=[
            QueryParam(
                name="period",
                type="str",
    ...
    def get_cashflow_summary(self, period: str) -> dict[str, Any]:
        """Get cash flow summary statistics for a period."""
        ...
        SELECT 
            ...
            AVG(n_cashflow_inv_act) as avg_investing_cf,
            AVG(n_cash_flows_fnc_act) as avg_financing_cf
        FROM ods_cash_flow
        ...
        return {
            ...
            "avg_operating_cf": float(row["avg_operating_cf"])
            if row["avg_operating_cf"]
            else None,
            "avg_investing_cf": float(row["avg_investing_cf"])
            if row["avg_investing_cf"]
            else None,
            "avg_financing_cf": float(row["avg_financing_cf"])
            if row["avg_financing_cf"]
            else None,
        }
