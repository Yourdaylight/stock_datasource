"""TuShare cash flow VIP data extractor."""
...
class CashflowVipExtractor:
    """Extractor for TuShare cash flow VIP data (batch mode)."""
    ...
    def _call_api(self, **kwargs) -> pd.DataFrame:
        """Call TuShare cashflow_vip API with rate limiting and retry."""
        self._rate_limit()
        
        try:
            result = self.pro.cashflow_vip(**kwargs)
    ...
    def extract(self, ts_code: Optional[str] = None, ann_date: Optional[str] = None,
                f_ann_date: Optional[str] = None, start_date: Optional[str] = None,
                end_date: Optional[str] = None, period: Optional[str] = None,
                report_type: Optional[str] = None, comp_type: Optional[str] = None,
                is_calc: Optional[int] = None) -> pd.DataFrame:
        """Extract cash flow VIP data.
        
        Args:
            ts_code: Stock code (optional for VIP, e.g., 600000.SH)
            ...
            period: Report period (e.g., 20231231) - main batch param
        ...
        if is_calc is not None:
            kwargs['is_calc'] = is_calc
        
        return self._call_api(**kwargs)


# Global extractor instance
extractor = CashflowVipExtractor()
