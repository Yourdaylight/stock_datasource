...
class TuShareIndexClassifyService(BaseService):
    ...
    @query_method(
        ...
        params=[
            QueryParam(
                name="level", type="str", description="Level L1/L2/L3", required=True
            ),
            QueryParam(
                name="src",
                type="str",
                description="Source SW2021/SW/MSCI",
                required=False,
            ),
        ],
    )
    def get_index_classify_by_level(
        self, level: str, src: str | None = None
    ) -> list[dict[str, Any]]:
        where = f"level = '{level}'"
        if src:
            where += f" AND src = '{src}'"
    ...
    def get_all_index_classify(self, src: str | None = None) -> list[dict[str, Any]]:
        where = "1=1"
        if src:
            where = f"src = '{src}'"
        query = f"""
