"""TuShare HK balance sheet data extractor."""
...
class HKBalancesheetExtractor:
    """Extractor for TuShare HK balance sheet data (EAV model)."""
    ...
    def _call_api(self, **kwargs) -> pd.DataFrame:
        """Call TuShare HK balance sheet API with rate limiting and retry."""
        self._rate_limit()
        
        try:
            result = self.pro.hk_balancesheet(**kwargs)
    ...
    def extract(self, ts_code: str, period: Optional[str] = None,
        ...
        Returns:
            DataFrame with HK balance sheet data (EAV: ts_code, end_date, ind_name, ind_value)
        """
...
extractor = HKBalancesheetExtractor()
