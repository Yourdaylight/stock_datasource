...
class MultiAgentArena:
    ...
    def __init__(self, config: ArenaConfig, user_id: str = ""):
        ...
        # Sub-components
        self.discussion_orchestrator: AgentDiscussionOrchestrator | None = None
        self.competition_engine: StrategyCompetitionEngine | None = None
    ...
    async def initialize(self) -> None:
        ...
        await self.stream_processor.publish_system(
            f"## 初始化完成\n- {len(self.discussion_orchestrator.agents)} 个Agent就绪",
        )
    ...
    async def _generate_initial_strategies(self) -> None:
        ...
        for agent in self.discussion_orchestrator.get_all_agents():
            ...
            try:
                ...
                logger.warning(
                    f"Agent {agent.agent_id} failed to generate strategy: {e}"
                )
    ...
    async def _run_discussion_phase(self) -> None:
        ...
        for mode in modes:
            ...
            self.arena.strategies = (
                await self.discussion_orchestrator.refine_strategies(
                    strategies=self.arena.strategies,
    ...
    async def _run_backtest_phase(self) -> None:
        ...
        # Store results
        backtest_results = {strategy.id: result for strategy, result in results}
    ...
    def get_status(self) -> dict[str, Any]:
        ...
        return {
            ...
            "last_evaluation": self.arena.last_evaluation.isoformat()
            if self.arena.last_evaluation
            else None,
