...
class TuShareExtractor(BaseTuShareExtractor):
    ...
    def _get_trade_calendar_robust(
        self, start_date: str, end_date: str, exchange: str = "SSE"
    ) -> pd.DataFrame:
        """
        ...
        try:
            ...
            if not result.empty:
                ...
                if not result_filtered.empty:
                    logger.info(
                        f"Trade calendar: got {len(result_filtered)} records for {start_date} to {end_date}"
                    )
        ...
        if start_year == end_year:
            ...
            logger.info(
                f"Single year request detected, extending range to {extended_end}"
            )
            ...
            try:
                ...
                if not result.empty:
                    ...
                    if not result_filtered.empty:
                        logger.info(
                            f"Trade calendar (extended): got {len(result_filtered)} records for {start_date} to {end_date}"
                        )
                        return result_filtered
            except Exception as e:
            ...
            try:
                ...
                if not year_result.empty:
                    ...
                    if not year_filtered.empty:
                        ...
                        logger.info(
                            f"Trade calendar (year {year}): got {len(year_filtered)} records"
                        )
        ...
        if all_data:
            ...
            logger.info(
                f"Trade calendar (combined): got {len(final_result)} records for {start_date} to {end_date}"
            )
            return final_result

        # Strategy 4: Fallback - create a basic calendar (weekdays only)
        logger.warning(
            f"All trade calendar strategies failed, creating basic weekday calendar for {start_date} to {end_date}"
        )
        return self._create_fallback_calendar(start_date, end_date, exchange)

    def _create_fallback_calendar(
        self, start_date: str, end_date: str, exchange: str = "SSE"
    ) -> pd.DataFrame:
    ...
    def get_stock_basic(
        self, list_status: str = "L", fields: list[str] | None = None
    ) -> pd.DataFrame:
        ...
        return self._call_api(
            self.pro.stock_basic, list_status=list_status, fields=",".join(fields)
        )
    ...
    def get_hk_basic(self, list_status: str = "L") -> pd.DataFrame:
        ...
        logger.warning(
            "HK basic data extraction is limited to 10 calls/day - using placeholder"
        )
    ...
    def get_hk_daily(self, trade_date: str, ts_code: str | None = None) -> pd.DataFrame:
        """Get Hong Kong daily data (placeholder)."""
        # HK data has very limited API calls (10/day), so this is mainly for structure
        logger.warning(
            "HK daily data extraction is limited to 10 calls/day - using placeholder"
        )
    ...
    def batch_extract_daily_data(
        self, start_date: str, end_date: str, ts_codes: list[str] | None = None
    ) -> pd.DataFrame:
        ...
        if trade_cal.empty or "is_open" not in trade_cal.columns:
            logger.error(
                f"Trade calendar is empty or missing 'is_open' column for {start_date} to {end_date}"
            )
            return pd.DataFrame()
        ...
        for trade_date in trade_dates:
            try:
                ...
                logger.info(
                    f"Extracted daily data for {trade_date}: {len(daily_data)} records"
                )
    ...
    def extract_all_data_for_date(
        self, trade_date: str, check_schedule: bool = True, is_backfill: bool = False
                ...
                if table_name and db_client.table_exists(table_name):
                    try:
                        ...
                        if record_count > 0:
                            ...
                            logger.info(
                                f"Plugin {plugin_name} data already exists for {trade_date}, skipping"
                            )
                            should_skip_by_schedule = True
                        else:
                            # No data for this date, extract regardless of schedule
                            logger.info(
                                f"Plugin {plugin_name} no data for {trade_date}, extracting for backfill"
                            )
                    except Exception as e:
                        logger.warning(
                            f"Failed to check table {table_name} for date {trade_date}: {e}, will extract"
                        )
                else:
                    # Table doesn't exist, extract
                    logger.info(
                        f"Plugin {plugin_name} table doesn't exist, extracting for backfill"
                    )
                ...
                if table_name and db_client.table_exists(table_name):
                    try:
                        ...
                        if record_count > 0:
                            ...
                            logger.info(
                                f"Plugin {plugin_name} not scheduled for {trade_date} and table has data, skipping"
                            )
                            should_skip_by_schedule = True
                        else:
                            # Table is empty, must extract
                            logger.info(
                                f"Plugin {plugin_name} not scheduled for {trade_date} but table is empty, extracting anyway"
                            )
                    except Exception as e:
                        logger.warning(
                            f"Failed to check table {table_name} data count: {e}, will extract"
                        )
                else:
                    # Table doesn't exist, skip
                    logger.info(
                        f"Plugin {plugin_name} not scheduled for {trade_date}, skipping"
                    )
                    should_skip_by_schedule = True
    ...
    def validate_data_quality(
        self, data: pd.DataFrame, expected_date: str
    ) -> dict[str, Any]:
        ...
        if "trade_date" in data.columns:
            ...
            if len(unique_dates) > 1:
                ...
                issues.append(
                    f"Trade date mismatch: expected {expected_date}, got {unique_dates[0]}"
                )
