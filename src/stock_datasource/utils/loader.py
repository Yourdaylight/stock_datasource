...
class DataLoader:
    ...
    def load_ods_data(
        self, table_name: str, data: pd.DataFrame, api_name: str
    ) -> dict[str, Any]:
        """
        ...
        for col_name in data.columns:
            ...
            if "Date" in target_type and col_name in data.columns:
                ...
                try:
                    ...
                    logger.warning(
                        f"Failed to convert {col_name} to date: {e}, trying alternative format"
                    )
    ...
    def load_dim_security(self, stock_basic_data: pd.DataFrame) -> dict[str, Any]:
        ...
        available_cols = [col for col in column_mapping if col in dim_data.columns]
    ...
    def load_fact_daily_bar(
        self, daily_data: pd.DataFrame, adj_factor_data: pd.DataFrame | None = None
    ) -> dict[str, Any]:
        ...
        try:
            ...
            stats = {
                ...
                "status": "success",
                "has_adj_factors": adj_factor_data is not None
                and not adj_factor_data.empty,
            }
    ...
    def process_daily_ingestion(
        self, trade_date: str, extracted_data: dict[str, pd.DataFrame]
    ) -> dict[str, Any]:
        ...
        try:
            ...
            if "daily" in extracted_data and not extracted_data["daily"].empty:
                try:
                    daily_stats = self.load_fact_daily_bar(
                        extracted_data["daily"], extracted_data.get("adj_factor")
                    )
            ...
            if (
                "stock_basic" in extracted_data
                and not extracted_data["stock_basic"].empty
            ):
                try:
                    security_stats = self.load_dim_security(
                        extracted_data["stock_basic"]
                    )
            ...
            logger.info(
                f"Daily ingestion completed for {trade_date}: "
    ...
    def get_loading_statistics(
        ...
        # Build query
        query = "SELECT COUNT(*) as total_records, MAX(_ingested_at) as last_load"
    ...
    def cleanup_old_versions(
        self, table_name: str, keep_days: int = 30
    ) -> dict[str, Any]:
        """Clean up old versions from ReplacingMergeTree tables."""
        logger.info(
            f"Cleaning up old versions in {table_name}, keeping {keep_days} days"
        )
        ...
        try:
            ...
            return {"table_name": table_name, "error": str(e), "status": "failed"}
