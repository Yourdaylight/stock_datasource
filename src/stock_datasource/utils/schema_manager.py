...
class SchemaManager:
    ...
    def sync_schema_from_api(
        self, table_name: str, api_data: pd.DataFrame, api_name: str
    ) -> bool:
        """
        ...
        for col_name, (current_type, new_type) in type_mismatches.items():
            if self._is_widening_conversion(current_type, new_type):
                ...
                schema_changed = True
                logger.info(
                    f"Modified column {col_name} in {table_name} from {current_type} to {new_type}"
                )
            else:
                ...
                logger.warning(
                    f"Incompatible type change for {col_name}: {current_type} -> {new_type}"
                )
    ...
    def _infer_schema_from_data(
        ...
        for col_name in data.columns:
            ...
            columns.append(
                ColumnDefinition(name=col_name, data_type=data_type, nullable=True)
            )
    ...
    def _infer_clickhouse_type(self, series: pd.Series) -> str:
        ...
        if pd.api.types.is_integer_dtype(series):
            ...
            if series.name and any(
                keyword in series.name.lower()
                for keyword in ["code", "symbol", "ticker"]
            ):
    ...
    def _check_type_mismatches(
        self, current_schema: list[dict], api_data: pd.DataFrame
    ) -> dict[str, tuple]:
        ...
        for col_info in current_schema:
            ...
            if col_name not in api_data.columns or col_name in [
                "version",
                "_ingested_at",
            ]:
                continue
    ...
    def _is_widening_conversion(self, current_type: str, new_type: str) -> bool:
        ...
        if current_type.startswith("Nullable(") and not new_type.startswith(
            "Nullable("
        ):
    ...
    def _log_schema_change(
        self, table_name: str, change_type: str, change_details: str
    ) -> None:
