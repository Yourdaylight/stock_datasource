...
class DataValidator:
    ...
    def validate_not_empty(
        self, data: pd.DataFrame, context: str = ""
    ) -> tuple[bool, str | None]:
    ...
    def validate_required_columns(
        self, data: pd.DataFrame, required_cols: list[str], context: str = ""
    ) -> tuple[bool, str | None]:
    ...
    def validate_no_null_keys(
        self, data: pd.DataFrame, key_cols: list[str], context: str = ""
    ) -> tuple[bool, str | None]:
    ...
    def validate_no_duplicates(
        self, data: pd.DataFrame, subset: list[str], context: str = ""
    ) -> tuple[bool, str | None]:
    ...
    def validate_column_types(
        self, data: pd.DataFrame, type_map: dict[str, str], context: str = ""
    ) -> tuple[bool, str | None]:
    ...
    def validate_positive_values(
        self, data: pd.DataFrame, cols: list[str], context: str = ""
    ) -> tuple[bool, str | None]:
    ...
    def validate_price_change_consistency(
        self, data: pd.DataFrame, context: str = ""
    ) -> tuple[bool, str | None]:
        """Validate price change and percentage change consistency."""
        if not all(
            col in data.columns for col in ["close", "pre_close", "change", "pct_chg"]
        ):
            return True, None
    ...
    def validate_volume_consistency(
        ...
        if valid_rows.sum() > 0:
            expected_amount = (
                data.loc[valid_rows, "vol"] * data.loc[valid_rows, "close"]
            )
    ...
    def validate_daily_data(
        self, data: pd.DataFrame, trade_date: str
    ) -> dict[str, Any]:
        ...
        checks = [
            ...
            (
                "no_null_keys",
                self.validate_no_null_keys(data, ["ts_code", "trade_date"], context),
            ),
            (
                "no_duplicates",
                self.validate_no_duplicates(data, ["ts_code", "trade_date"], context),
            ),
        ]
                ...
                [
                    (
                        ...
                        self.validate_date_format(
                            data, ["trade_date"], context=context
                        ),
                    ),
                    (
                        "positive_prices",
                        self.validate_positive_values(
                            data, ["open", "high", "low", "close", "pre_close"], context
                        ),
                    ),
                    (
                        "positive_volume",
                        self.validate_positive_values(data, ["vol", "amount"], context),
                    ),
                    (
                        "ohlc_relationship",
                        self.validate_ohlc_relationship(data, context),
                    ),
                    (
                        "price_change_consistency",
                        self.validate_price_change_consistency(data, context),
                    ),
                    (
                        "volume_consistency",
                        self.validate_volume_consistency(data, context),
                    ),
                ]
            )
        ...
        for check_name, (passed, issue) in checks:
            result["checks"].append(
                {"name": check_name, "passed": passed, "issue": issue}
            )
            if not passed:
                result["passed"] = False
    ...
    def validate_adj_factor_data(
        ...
        checks = [
            ...
            (
                "required_columns",
                self.validate_required_columns(
                    data, ["ts_code", "trade_date", "adj_factor"], context
                ),
            ),
            (
                "no_null_keys",
        ...
        if not data.empty:
            checks.extend(
                [
                    ...
                    (
                        "positive_adj_factor",
                        self.validate_positive_values(data, ["adj_factor"], context),
                    ),
                ]
            )
