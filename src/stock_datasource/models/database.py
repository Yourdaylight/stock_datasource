...
class ClickHouseHttpClient:
    ...
    def _request(self, query: str, params: dict | None = None, data: str = None) -> str:
        ...
        if params:
            ...
            if unreplaced:
                raise ValueError(
                    f"Unbound ClickHouse parameters in HTTP query: {unreplaced}"
                )
        ...
        if data:
            ...
            resp = self._session.post(
                self._base_url, params=req_params, data=data, timeout=60
            )
        else:
    ...
    def _extract_unknown_table(exc: Exception) -> str | None:
    ...
    def insert_dataframe(
        self, table_name: str, df: pd.DataFrame, settings: dict | None = None
    ) -> None:
        ...
        with self._lock:
            try:
                ...
                logger.error(
                    f"Failed to insert data into {table_name} [{self.name}]: {e}"
                )
                raise
    ...
    def _try_auto_create_table(self, table_name: str) -> bool:
        ...
        try:
            ...
            if create_sql:
                logger.info(
                    f"Auto-creating table {table_name} from registered schema [{self.name}]"
                )
                self._request(create_sql)
                logger.info(
                    f"Table {table_name} auto-created successfully [{self.name}]"
                )
                return True
        except Exception as e:
            logger.warning(
                f"Failed to auto-create table {table_name} [{self.name}]: {e}"
            )
        return False

    @staticmethod
    def _build_create_table_sql(table_name: str, schema: dict[str, Any]) -> str | None:
    ...
    def table_exists(self, table_name: str) -> bool:
        ...
        result = self._request(
            query, params={"database": self.database, "table_name": table_name}
        )
...
class ClickHouseClient:
    ...
    def __init__(
        ...
        self.http_port = (
            http_port
            if http_port != 8123
            else getattr(settings, "CLICKHOUSE_HTTP_PORT", 8123)
        )
        self.client = None
    ...
    def _connect(self):
        ...
        try:
            ...
            logger.info(
                f"Connected to ClickHouse via TCP [{self.name}]: {self.host}:{self.port}"
            )
            self._use_http = False
        except Exception as e:
            logger.warning(
                f"TCP connection failed [{self.name}]: {e}, falling back to HTTP"
            )
            self._init_http_client()
    ...
    def _reconnect(self):
        ...
        try:
            ...
            logger.error(
                f"Reconnect to ClickHouse failed [{self.name}]: {reconnect_err}"
            )
            raise
    ...
    def _try_auto_create_table(self, table_name: str) -> bool:
        ...
        try:
            create_sql = ClickHouseHttpClient._build_create_table_sql(
                table_name, schema
            )
            if create_sql:
                logger.info(
    ...
    @retry(
        stop=stop_after_attempt(2), wait=wait_exponential(multiplier=0.5, min=1, max=3)
    )
    ...
    def execute_query(self, query: str, params: dict | None = None) -> pd.DataFrame:
        ...
        with self._lock:
            ...
            try:
                ...
                if self._should_reconnect(e):
                    logger.warning(
                        f"Reconnect ClickHouse during query_dataframe [{self.name}] due to: {e}"
                    )
                    self._reconnect()
                    # After reconnect, check if we switched to HTTP
                    if self._use_http and self._http_client:
        ...
        with self._lock:
            ...
            try:
                self.client.insert_dataframe(
                    f"INSERT INTO {table_name} VALUES", df, settings=settings or {}
                )
                logger.info(f"Inserted {len(df)} rows into {table_name} [{self.name}]")
            except Exception as e:
                if self._should_reconnect(e):
                    logger.warning(
                        f"Reconnect ClickHouse during insert [{self.name}] due to: {e}"
                    )
                    self._reconnect()
                    # After reconnect, check if we switched to HTTP
                    if self._use_http and self._http_client:
                    ...
                    logger.info(
                        f"Inserted {len(df)} rows into {table_name} [{self.name}]"
                    )
                else:
                    logger.error(
                        f"Failed to insert data into {table_name} [{self.name}]: {e}"
    ...
    def modify_column(self, table_name: str, column_name: str, new_type: str) -> None:
        ...
        self.execute(query)
        logger.info(
            f"Modified column {column_name} in {table_name} to {new_type} [{self.name}]"
        )
    ...
    def __init__(self):
        ...
        if settings.BACKUP_CLICKHOUSE_HOST:
            try:
                ...
                logger.info(
                    f"Dual write enabled: backup at {settings.BACKUP_CLICKHOUSE_HOST}"
                )
            except Exception as e:
                logger.warning(
                    f"Failed to connect to backup ClickHouse, dual write disabled: {e}"
                )
                self.backup = None
