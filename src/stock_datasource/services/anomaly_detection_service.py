...
class AnomalyDetectionService:
    ...
    async def detect_volume_anomalies(
        ...
        try:
            # 获取最近的交易日
            latest_date_query = (
                "SELECT MAX(trade_date) as latest_date FROM ods_top_list"
            )
    ...
    async def detect_seat_concentration_anomalies(
        ...
        try:
            # 获取最近的交易日
            latest_date_query = (
                "SELECT MAX(trade_date) as latest_date FROM ods_top_inst"
            )
            latest_result = self.db.query(latest_date_query)
            latest_result = self._convert_df_to_list(latest_result)
            ...
            result = self.db.query(
                query, {"latest_date": latest_date, "threshold": threshold}
            )
    ...
    async def detect_price_momentum_anomalies(
        self, days: int = 5
    ) -> list[dict[str, Any]]:
        """检测价格动量异动
        ...
        try:
            ...
            for item in result:
                ...
                item["description"] = (
                    f"价格动量异常(累计变动{float(item.get('cumulative_change', 0)):.1f}%)"
                )
    ...
    async def detect_institutional_behavior_anomalies(self) -> list[dict[str, Any]]:
        ...
        try:
            ...
            for item in result:
                ...
                item["description"] = (
                    f"机构{direction}{net_flow / 10000:.1f}万元({inst_count}个席位)"
                )
    ...
    async def generate_anomaly_alerts(self) -> list[AnomalyAlert]:
        ...
        try:
            ...
            institutional_anomalies = (
                await self.detect_institutional_behavior_anomalies()
            )
    ...
    def _calculate_confidence(self, anomaly: dict[str, Any]) -> float:
        ...
        # 根据严重程度调整
        severity_bonus = {"critical": 0.1, "high": 0.05, "medium": 0.0, "low": -0.05}
