...
class IngestionService:
    ...
    def ingest_daily_data(
        ...
        try:
            ...
            extracted_data = self.extractor.extract_all_data_for_date(
                trade_date, check_schedule=check_schedule, is_backfill=False
            )
            ...
            loading_result = self.loader.process_daily_ingestion(
                trade_date, extracted_data
            )
            ...
            if loading_result["status"] == "failed" or (
                qc_result and qc_result.get("overall_status") == "failed"
            ):
            ...
            logger.info(
                f"Daily ingestion completed for {trade_date}: {result['status']}, "
    ...
    def backfill_data(
        self, start_date: str, end_date: str, run_quality_checks: bool = True
    ) -> dict[str, Any]:
        """
        ...
        task_id = str(uuid4())
        logger.info(
            f"Starting backfill from {start_date} to {end_date}, task_id: {task_id}"
        )
        ...
        result = {
            ...
            "status": "running",
            "dates_processed": [],
            "summary": {"total_dates": 0, "successful": 0, "warnings": 0, "failed": 0},
            "error": None,
        }
        ...
        try:
            ...
            for trade_date in trade_dates:
                try:
                    ...
                    daily_result = self._ingest_daily_data_backfill(
                        trade_date, run_quality_checks
                    )
                    ...
                    logger.info(
                        f"Backfill progress: {trade_date} - {daily_result['status']}"
                    )
            ...
            logger.info(
                f"Backfill completed: {result['status']}, "
    ...
    def _ingest_daily_data_backfill(
        self, trade_date: str, run_quality_checks: bool = True
    ) -> dict[str, Any]:
        """
        ...
        task_id = str(uuid4())
        logger.info(
            f"Starting backfill daily ingestion for {trade_date}, task_id: {task_id}"
        )
        ...
        try:
            ...
            if loading_result["status"] == "failed":
                ...
                result["status"] = "warning"
                logger.warning(
                    f"Quality checks failed for {trade_date} but treating as warning for backfill"
                )
            ...
            logger.info(
                f"Backfill daily ingestion completed for {trade_date}: {result['status']}, "
                f"duration: {result['duration_seconds']:.2f}s"
            )
    ...
    def _run_backfill_quality_checks(self, trade_date: str) -> dict[str, Any]:
        ...
        try:
            ...
            for check in result["checks"]:
                if check["check_name"] == "trade_date_alignment":
                    ...
                    if check["status"] == "failed":
                        ...
                        check["backfill_note"] = (
                            "Converted from failed to warning for backfill"
                        )
            ...
            result["summary"] = {
                ...
                "skipped": len(
                    [c for c in modified_checks if c["status"] == "skipped"]
                ),
    ...
    def cleanup_old_data(self, days_to_keep: int = 90) -> dict[str, Any]:
        ...
        try:
            ...
            for table in tables_to_clean:
                try:
                    if self.db.table_exists(table):
                        cleanup_stats = self.loader.cleanup_old_versions(
                            table, days_to_keep
                        )
            ...
            result["status"] = "success"
            logger.info(
                f"Cleanup completed: {len(result['tables_cleaned'])} tables cleaned"
            )
