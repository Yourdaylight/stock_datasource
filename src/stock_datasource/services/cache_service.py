...
class CacheService:
    ...
    def _get_redis(self):
        ...
        if self._redis is not None:
            ...
            if (
                self._available
                and _time.time() - self._last_ping_ok > self._PING_INTERVAL
            ):
                try:
                    self._redis.ping()
                    ...
                    logger.warning(
                        "CacheService Redis ping failed, marking unavailable"
                    )
    ...
    def set(self, key: str, value: Any, ttl: int = 300) -> bool:
        ...
        if redis is None:
            return False
        try:
            return bool(redis.setex(self._key(key), ttl, self._serialize(value)))
        except Exception as e:
    ...
    async def adelete_pattern(self, pattern: str) -> int:
        ...
        return await asyncio.get_event_loop().run_in_executor(
            None, self.delete_pattern, pattern
        )
