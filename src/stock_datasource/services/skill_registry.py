...
class SkillRegistry:
    ...
    def register(self, descriptor: SkillDescriptor) -> None:
        ...
        logger.debug(
            "Skill registered: %s (source=%s)", descriptor.name, descriptor.source
        )
    ...
    def list_skills(
        self, *, category: str = None, source: str = None, enabled_only: bool = True
    ...
    def find_by_tool(self, tool_name: str) -> list[SkillDescriptor]:
        return [
            s for s in self._skills.values() if tool_name in s.tool_refs and s.enabled
        ]
