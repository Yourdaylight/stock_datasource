...
class MetadataService:
    ...
    def log_ingestion_start(self, task_id: str, api_name: str, table_name: str) -> None:
        ...
        try:
            ...
            logger.info(
                f"Logged ingestion start: {task_id} - {api_name} - {table_name}"
            )
    ...
    def update_retry_count(self, task_id: str, retry_count: int) -> None:
        ...
        try:
            ...
            params = {"retry_count": retry_count, "task_id": task_id}
    ...
    def get_ingestion_statistics(
        ...
        try:
            ...
            for _, row in result.iterrows():
                ...
                stats["by_table"][table_name]["total_records"] += (
                    row["total_records"] or 0
                )

                if status == "success":
                    stats["by_table"][table_name]["successful_tasks"] += row[
                        "task_count"
                    ]
                elif status == "failed":
    ...
    def get_data_coverage(self, table_name: str) -> dict[str, Any]:
        ...
        try:
            ...
            expected_dates = (
                trade_cal_result.iloc[0]["expected_dates"]
                if not trade_cal_result.empty
                else 0
            )
            ...
            coverage_pct = (
                (actual_dates / expected_dates * 100) if expected_dates > 0 else 0
            )
    ...
    def get_schema_evolution_history(
    ...
    def get_quality_check_summary(
        ...
        try:
            ...
            summary = {
                "period": {"start_date": start_date, "end_date": end_date},
                "by_check": {},
                "overall": {"total_checks": 0, "passed": 0, "warning": 0, "failed": 0},
            }
            ...
            for _, row in result.iterrows():
                ...
                summary["by_check"][check_name]["unique_dates"] = max(
                    summary["by_check"][check_name]["unique_dates"], row["unique_dates"]
                )
    ...
    def generate_daily_report(self, report_date: str) -> dict[str, Any]:
        ...
        try:
            ...
            report = {
                ...
                "failed_tasks": {"count": len(failed_tasks), "tasks": failed_tasks},
                "summary": {"overall_status": "success", "issues": []},
            }
            ...
            if failed_tasks:
                ...
                report["summary"]["issues"].append(
                    f"{len(failed_tasks)} failed tasks need retry"
                )

            logger.info(
                f"Daily report generated for {report_date}: {report['summary']['overall_status']}"
            )
            return report
