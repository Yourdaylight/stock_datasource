...
class DataPipeline:
        ...
        for plugin in enabled_plugins:
            try:
                ...
                if data is not None and not data.empty:
                    ...
                    self.logger.info(
                        f"Extracted {len(data)} records from {plugin.name}"
                    )
                else:
                    result["plugins"][plugin.name] = {"records": 0, "status": "no_data"}
                    self.logger.warning(f"No data extracted from {plugin.name}")

            except Exception as e:
                result["plugins"][plugin.name] = {"status": "failed", "error": str(e)}
    ...
    def validate_daily_data(
        self, data_dict: dict[str, pd.DataFrame], trade_date: str
    ) -> dict[str, Any]:
        ...
        for source_name, data in data_dict.items():
            try:
                if "adj_factor" in source_name:
                    validation = self.validator.validate_adj_factor_data(
                        data, trade_date
                    )
                else:
                    validation = self.validator.validate_daily_data(data, trade_date)
                ...
                result["summary"]["total"] += 1
                result["summary"]["failed"] += 1
                result["summary"]["issues"].append(f"{source_name}: {e!s}")
    ...
    def load_daily_data(self, data_dict: dict[str, pd.DataFrame]) -> dict[str, Any]:
        ...
        result = {
            "start_time": datetime.now(),
            "tables": {},
            "summary": {"total": 0, "successful": 0, "failed": 0, "total_records": 0},
        }
            ...
            try:
                ...
                if load_result["tables"].get(table_name, {}).get("status") == "success":
                    ...
                    result["summary"]["total_records"] += (
                        load_result["tables"].get(table_name, {}).get("records", 0)
                    )
                else:
                    result["summary"]["failed"] += 1

            except Exception as e:
                result["tables"][table_name] = {"status": "failed", "error": str(e)}
                result["summary"]["total"] += 1
                result["summary"]["failed"] += 1
    ...
    def run_daily_pipeline(
        self, trade_date: str, skip_validation: bool = False
    ) -> dict[str, Any]:
        ...
        try:
            ...
            pipeline_result["summary"]["total_records_extracted"] = extraction_result[
                "total_records"
            ]
                ...
                if plugin_result.get("status") in ["success", "warning"]:
                    ...
                    if plugin:
                        try:
                            ...
                            self.logger.warning(
                                f"Failed to re-extract data from {plugin_name}: {e}"
                            )
            ...
            if not skip_validation:
                ...
                pipeline_result["summary"]["validation_issues"] = validation_result[
                    "summary"
                ]["failed"]
                ...
                if validation_result["summary"]["failed"] > 0:
                    ...
                    self.logger.warning(
                        f"Validation found {validation_result['summary']['failed']} issues"
                    )
            ...
            pipeline_result["summary"]["total_records_loaded"] = loading_result[
                "summary"
            ]["total_records"]
            ...
            if loading_result["summary"]["failed"] > 0:
                pipeline_result["status"] = "warning"
                self.logger.warning(
                    f"Loading had {loading_result['summary']['failed']} failures"
                )
