...
from stock_datasource.plugins.tushare_hk_balancesheet.service import (
    TuShareHKBalancesheetService,
)
from stock_datasource.plugins.tushare_hk_cashflow.service import (
    TuShareHKCashflowService,
)
from stock_datasource.plugins.tushare_hk_fina_indicator.service import (
    TuShareHKFinaIndicatorService,
)
...
class HKFinancialReportService:
    ...
    def get_comprehensive_analysis(
        ...
        try:
            ...
            income_data = self.income_service.get_income_pivot(
                code, periods_count=periods
            )
            balance_data = self.balancesheet_service.get_balancesheet_pivot(
                code, periods_count=periods
            )
            cashflow_data = self.cashflow_service.get_cashflow_pivot(
                code, periods_count=periods
            )
    ...
    def get_income_statement(
        self, code: str, periods: int = 8, period: str | None = None
    ) -> dict[str, Any]:
        """获取港股利润表数据（PIVOT 宽表格式）"""
        try:
            data = self.income_service.get_income_pivot(
                code, period=period, periods_count=periods
            )
            return _sanitize_json_data(
                {
                    "code": code,
    ...
    def get_balance_sheet(
        ...
        try:
            data = self.balancesheet_service.get_balancesheet_pivot(
                code, period=period, periods_count=periods
            )
            return _sanitize_json_data(
                {
    ...
    def get_cash_flow(
        ...
        try:
            data = self.cashflow_service.get_cashflow_pivot(
                code, period=period, periods_count=periods
            )
            return _sanitize_json_data(
                {
    ...
    def get_income_raw(
        ...
        """获取港股利润表原始 EAV 数据"""
        try:
            data = self.income_service.get_income(
                code, period=period, indicators=indicators, limit=limit
            )
    ...
    def get_balancesheet_raw(
        ...
        """获取港股资产负债表原始 EAV 数据"""
        try:
            data = self.balancesheet_service.get_balancesheet(
                code, period=period, indicators=indicators, limit=limit
            )
            return {"code": code, "count": len(data), "data": data, "status": "success"}
        except Exception as e:
    ...
    def list_income_indicators(self, code: str | None = None) -> list[str]:
        """列出港股利润表所有指标名称"""
        return self.income_service.list_indicators(code)

    def list_balancesheet_indicators(self, code: str | None = None) -> list[str]:
        """列出港股资产负债表所有指标名称"""
        return self.balancesheet_service.list_indicators(code)

    def list_cashflow_indicators(self, code: str | None = None) -> list[str]:
        """列出港股现金流量表所有指标名称"""
    ...
    def _build_summary(
        self, indicators: list[dict], profitability: dict
    ) -> dict[str, Any]:
    ...
    def _analyze_health(
        self, indicators: list[dict], balance_data: dict
    ) -> dict[str, Any]:
        ...
        if npr_val is not None:
            if npr_val > 25:
                score += 5
                strengths.append(
                    f"净利率优秀({npr_val:.1f}%)，费用管控和盈利转化能力强"
                )
        ...
        if np_yoy_val is not None:
            if np_yoy_val > 50:
                ...
                score -= 10
                weaknesses.append(
                    f"净利润同比大幅下滑({np_yoy_val:.1f}%)，盈利能力明显恶化"
                )
        ...
        if rev_yoy_val is not None:
            if rev_yoy_val > 30:
                score += 5
                strengths.append(
                    f"营收同比高速增长({rev_yoy_val:.1f}%)，市场扩张势头强劲"
                )
            elif rev_yoy_val > 10:
                score += 3
                ...
                score -= 8
                weaknesses.append(
                    f"营收同比下滑({rev_yoy_val:.1f}%)，市场竞争力或需求减弱"
                )
        ...
        if len(indicators) >= 3:
            ...
            if len(roe_trend) >= 3:
                if all(
                    roe_trend[i] >= roe_trend[i + 1] for i in range(len(roe_trend) - 1)
                ):
                ...
                if prev_rev > 0:
                    growth[cur.get("end_date", f"period_{i}")] = {
                        "revenue_growth": round(
                            (cur_rev - prev_rev) / prev_rev * 100, 2
                        ),
                    }
