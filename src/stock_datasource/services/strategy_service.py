...
class StrategyService:
    ...
    async def validate_strategy_code(self, strategy_code: str) -> dict[str, Any]:
        ...
        try:
            ...
            return {"valid": False, "error": f"语法错误: {e!s}"}
        except Exception as e:
            return {"valid": False, "error": f"验证失败: {e!s}"}
    ...
    async def register_custom_strategy(
        self, name: str, strategy_code: str, overwrite: bool = False
    ) -> dict[str, Any]:
        """注册自定义策略"""
        try:
            # 验证策略代码
            ...
            if not strategy_class:
                return {"success": False, "error": "未找到有效的策略类"}

            # 注册策略
            self.registry.register_strategy(name, strategy_class, overwrite=overwrite)

            return {"success": True, "message": f"策略 {name} 注册成功"}

        except Exception as e:
            return {"success": False, "error": f"注册失败: {e!s}"}

    async def unregister_strategy(self, name: str) -> dict[str, Any]:
        """注销策略"""
        try:
            self.registry.unregister_strategy(name)
            return {"success": True, "message": f"策略 {name} 注销成功"}
        except Exception as e:
            return {"success": False, "error": f"注销失败: {e!s}"}
    ...
    async def get_strategy_performance_summary(
        ...
        for symbol in symbols:
            try:
                ...
                results[symbol] = {"success": True, "result": result.dict()}
        ...
        return {"summary": summary_stats, "detailed_results": results}
