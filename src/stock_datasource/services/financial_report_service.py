...
from stock_datasource.plugins.tushare_balancesheet.service import (
    TuShareBalancesheetService,
)
...
from stock_datasource.plugins.tushare_finace_indicator.service import (
...
class FinancialReportService:
    ...
    def get_comprehensive_analysis(
        self, code: str, periods: int | None = 4
    ) -> dict[str, Any]:
        ...
        try:
            ...
            # Get growth analysis
            growth_analysis = self.finace_service.calculate_growth_rates(
                code, periods * 2
            )

            # Get financial health analysis
            health_analysis = self.finace_service.analyze_financial_health(
                code, periods
            )
    ...
    def _supplement_income_data(
        self, summary: dict[str, Any], code: str, periods: int
    ) -> None:
        ...
        try:
            ...
            income_result = self.income_service.get_profitability_metrics(
                code, periods * 3
            )
            ...
            for item in raw_data:
                ...
                if not self._is_valid_value(item.get("total_revenue")) and inc.get(
                    "total_revenue"
                ):
                    item["total_revenue"] = inc["total_revenue"]
                if not self._is_valid_value(item.get("net_profit")) and inc.get(
                    "net_income"
                ):
            ...
            for item in raw_data:
                ...
                ) and inc.get("gross_margin"):
                    item["gross_profit_margin"] = inc["gross_margin"]
                if not self._is_valid_value(item.get("net_profit_margin")) and inc.get(
                    "net_margin"
                ):
            ...
            if not growth.get("revenue_growth") and len(raw_data) >= 2:
                try:
                    curr_rev = (
                        float(raw_data[0].get("total_revenue"))
                        if self._is_valid_value(raw_data[0].get("total_revenue"))
                        else None
                    )
                    prev_rev = (
                        float(raw_data[1].get("total_revenue"))
                        if self._is_valid_value(raw_data[1].get("total_revenue"))
                        else None
                    )
            ...
            if not growth.get("profit_growth") and len(raw_data) >= 2:
                try:
                    curr_np = (
                        float(raw_data[0].get("net_profit"))
                        if self._is_valid_value(raw_data[0].get("net_profit"))
                        else None
                    )
                    prev_np = (
                        float(raw_data[1].get("net_profit"))
                        if self._is_valid_value(raw_data[1].get("net_profit"))
                        else None
                    )
    ...
    def get_peer_comparison_analysis(
        self, code: str, end_date: str | None = None
    ) -> dict[str, Any]:
    ...
    def get_financial_trends(
        self, code: str, periods: int | None = 8
    ) -> dict[str, Any]:
        ...
        try:
            ...
            insights = {
                "investment_thesis": self._generate_investment_thesis(
                    comprehensive, peer_comparison, trends
                ),
    ...
    def _interpret_peer_comparison(self, comparison: dict[str, Any]) -> dict[str, Any]:
        ...
        for metric, data in comparison["comparison"].items():
            ...
            interpretations[metric] = {
                ...
                "vs_industry": "高于行业中位数"
                if target_value and industry_median and target_value > industry_median
                else "低于行业中位数",
            }
        ...
        for metric in metrics:
            ...
            if len(values) >= 3:
                ...
                trends[metric] = {
                    ...
                    "change_pct": round(
                        ((recent_avg - earlier_avg) / earlier_avg * 100), 2
                    )
                    if earlier_avg != 0
                    else 0,
                }
    ...
    def _identify_risk_factors(
        self, comprehensive: dict[str, Any], trends: dict[str, Any]
    ) -> list[str]:
        ...
        for metric, trend_info in trend_data.items():
            if trend_info.get("trend") == "下降" and metric in [
                "roe",
                "net_profit_margin",
                "total_revenue",
            ]:
    ...
    def _assess_competitive_position(
        self, peer_comparison: dict[str, Any]
    ) -> dict[str, Any]:
        """Assess competitive position based on peer comparison."""
        interpretation = peer_comparison.get("interpretation", {})

        excellent_metrics = sum(
            1 for m in interpretation.values() if m.get("level") == "优秀"
        )
        good_metrics = sum(
            1 for m in interpretation.values() if m.get("level") == "良好"
        )
    ...
    def _assess_financial_strength(
        self, comprehensive: dict[str, Any]
    ) -> dict[str, Any]:
    ...
    def _assess_growth_prospects(self, trends: dict[str, Any]) -> dict[str, Any]:
        ...
        positive_trends = sum(
            1 for t in trend_data.values() if t.get("trend") == "上升"
        )
        ...
        return {
            ...
            "positive_trend_ratio": positive_trends / total_trends
            if total_trends > 0
            else 0,
        }
    ...
    def get_income_statement(
        self, code: str, periods: int = 4, report_type: int = 1
    ) -> dict[str, Any]:
        """获取利润表数据
