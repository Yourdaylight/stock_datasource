...
class TopListAnalysisService:
    ...
    async def analyze_institutional_flow(self, trade_date: str) -> dict[str, Any]:
        ...
        try:
            ...
            if not inst_df.empty:
                ...
                # 活跃机构（操作股票数量多）
                active_institutions = inst_summary[
                    inst_summary["stock_count"] >= 3
                ].head(10)
            ...
            self.logger.error(
                f"Failed to analyze institutional flow for {trade_date}: {e}"
            )
            raise
    ...
    async def identify_hot_money_targets(self, days: int = 5) -> list[dict[str, Any]]:
        ...
        try:
            ...
            self.logger.info(
                f"Identified {len(result)} hot money targets in last {days} days"
            )
            return result
    ...
    async def calculate_seat_concentration(
        self, ts_code: str, days: int = 10
    ) -> dict[str, Any]:
        """计算席位集中度
        ...
        try:
            ...
            top_seats = (
                df.groupby("exalter")
                .agg({"net_buy": "sum", "trade_date": "count", "seat_type": "first"})
                .reset_index()
            )
            ...
            stability_score = (
                (repeat_appearances > 1).sum() / len(repeat_appearances)
                if len(repeat_appearances) > 0
                else 0
            )
            ...
            if total_net_flow != 0:
                analysis["institution_dominance"] = float(
                    abs(institution_flow) / abs(total_net_flow)
                )
            else:
            ...
            self.logger.error(
                f"Failed to calculate seat concentration for {ts_code}: {e}"
            )
            raise
    ...
    async def analyze_sector_flow(self, trade_date: str) -> dict[str, Any]:
        ...
        try:
            ...
            for item in result:
                item["amount_ratio"] = (
                    (item["total_amount"] or 0) / total_amount
                    if total_amount > 0
                    else 0
                )
            ...
            analysis = {
                ...
                "net_inflow_sectors": len(
                    [r for r in result if (r["net_flow"] or 0) > 0]
                ),
                "net_outflow_sectors": len(
                    [r for r in result if (r["net_flow"] or 0) < 0]
                ),
            }
    ...
    async def detect_unusual_activity(self, days: int = 5) -> list[dict[str, Any]]:
        ...
        try:
            ...
            self.logger.info(
                f"Detected {len(anomalies)} unusual activities in last {days} days"
            )
