...
class Trade:
    ...
    def total_cost(self) -> float:
        """总成本（包含手续费和滑点）"""
        return (
            self.trade_value + self.commission + abs(self.slippage * self.trade_value)
        )
...
class BacktestResult:
    ...
    def get_trade_summary(self) -> dict[str, Any]:
        ...
        return {
            ...
            "buy_trades": len(
                [t for t in filled_trades if t.trade_type == TradeType.BUY]
            ),
            "sell_trades": len(
                [t for t in filled_trades if t.trade_type == TradeType.SELL]
            ),
            "total_commission": sum(t.commission for t in filled_trades),
            "total_slippage": sum(
                abs(t.slippage * t.trade_value) for t in filled_trades
            ),
            "avg_trade_size": np.mean([t.trade_value for t in filled_trades])
            if filled_trades
            else 0,
        }
...
class IntelligentBacktestResult(BacktestResult):
    ...
    optimized_result: BacktestResult | None = None
    robustness_results: list[BacktestResult] | None = None
    ...
    def get_best_result(self) -> BacktestResult:
        ...
        if (
            ...
            and self.optimized_result.performance_metrics.sharpe_ratio
