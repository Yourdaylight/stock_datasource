...
from .models import PerformanceMetrics, RiskMetrics, Trade, TradeStatus, TradeType
...
class PerformanceAnalyzer:
        ...
        if benchmark_returns is not None and len(benchmark_returns) > 0:
            ...
            if len(aligned_returns) > 1 and len(aligned_benchmark) > 1:
                ...
                expected_return = self.risk_free_rate + metrics.beta * (
                    benchmark_annualized - self.risk_free_rate
                )
    ...
    def _calculate_risk_metrics(
        self, returns: pd.Series, equity_curve: pd.Series, trading_days_per_year: int
    ) -> RiskMetrics:
        """计算风险指标"""
        metrics = RiskMetrics()
        ...
        if len(upside_returns) > 0:
            metrics.upside_deviation = upside_returns.std() * np.sqrt(
                trading_days_per_year
            )

        if len(downside_returns) > 0:
            metrics.downside_deviation = downside_returns.std() * np.sqrt(
                trading_days_per_year
            )
        ...
        if len(drawdown_series) > 0:
            ...
            if len(drawdown_periods) > 0:
                ...
                metrics.drawdown_frequency = len(drawdown_periods) / len(
                    drawdown_series
                )
    ...
    def generate_performance_report(
        self, performance_metrics: PerformanceMetrics, risk_metrics: RiskMetrics
    ) -> str:
        """生成绩效报告"""
        report = f"""
## 策略绩效报告
