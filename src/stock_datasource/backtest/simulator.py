...
from .models import Position, Trade, TradeStatus, TradeType, TradingConfig
...
class TradingSimulator:
    ...
    def __init__(self, config: TradingConfig):
        ...
        logger.info(
            f"Trading simulator initialized with capital: {config.initial_capital}"
        )

    def execute_signal(
        self, signal: TradingSignal, market_data: pd.Series
    ) -> Trade | None:
        """
        执行交易信号
        ...
        try:
            ...
            if not self._can_execute_trade(trade):
                logger.warning(
                    "Cannot execute trade: insufficient funds or position limits"
                )
                return None
            ...
            logger.info(
                f"Executed trade: {trade.trade_type.value} {trade.quantity} {trade.symbol} @ {trade.price}"
            )
            return trade
    ...
    def _calculate_trade_quantity(self, signal: TradingSignal, price: float) -> int:
        ...
        if signal.action == "buy":
            ...
            estimated_commission = max(
                max_investment * self.config.commission_rate, self.config.min_commission
            )
    ...
    def _execute_trade(self, trade: Trade) -> None:
        ...
        logger.debug(
            f"Trade executed: {trade.trade_type.value} {trade.quantity} {symbol}, Cash: {self.cash:.2f}"
        )
    ...
    def close_all_positions(
        ...
        for symbol, position in list(self.positions.items()):
            ...
            self.execute_signal(
                signal, row if row is not None else pd.Series({"close": close_price})
            )
