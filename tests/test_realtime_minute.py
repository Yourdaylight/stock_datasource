...
class TestSchemas:
    ...
    def test_batch_minute_data_response(self):
        from stock_datasource.modules.realtime_minute.schemas import (
...
class TestConfig:
    ...
    def test_rate_limit_config(self):
        ...
        assert pytest.approx(0.5, abs=0.01) == cfg.MIN_CALL_INTERVAL
...
class TestCollector:
    ...
    def collector(self):
        with patch(
            "stock_datasource.modules.realtime_minute.collector.settings"
        ) as mock_settings:
            mock_settings.TUSHARE_TOKEN = "test_token"
            with patch(
                "stock_datasource.modules.realtime_minute.collector.ts"
            ) as mock_ts:
                mock_ts.pro_api.return_value = MagicMock()
                from stock_datasource.modules.realtime_minute.collector import (
                    RealtimeMinuteCollector,
                )
    ...
    def test_collect_astock_with_mock(self, collector, sample_rt_min_df):
        ...
        with patch(
            "stock_datasource.modules.realtime_minute.collector.cfg"
        ) as mock_cfg:
...
class TestCacheStore:
    ...
    def test_store_bars_handles_pipeline_error(
        self, cache_store, mock_redis, sample_df
    ):
        mock_pipe = MagicMock()
        mock_redis.pipeline.return_value = mock_pipe
    ...
    def test_get_status_with_data(self, cache_store, mock_redis):
        mock_redis.hgetall.return_value = {
            "a_stock": json.dumps(
                {"last_collect_time": "2026-03-01 10:00:00", "records": 100}
            ),
        }
        result = cache_store.get_status()
        assert "a_stock" in result
...
class TestSyncService:
    ...
    def sync_service(self):
        from stock_datasource.modules.realtime_minute.sync_service import (
            RealtimeMinuteSyncService,
        )
    ...
    def test_ensure_table_creates_table(self, sync_service):
        with (
            patch(
                "stock_datasource.modules.realtime_minute.sync_service._get_db"
            ) as mock_get_db,
    ...
    def test_sync_no_data(self, sync_service):
        ...
        with patch(
            "stock_datasource.modules.realtime_minute.sync_service.get_cache_store"
        ) as mock_cache:
    ...
    def test_sync_multi_market(self, sync_service):
        ...
        with patch(
            ...
            with patch(
                ...
                tables_written = {
                    c[0][0] for c in mock_db.insert_dataframe.call_args_list
                }
...
class TestService:
    ...
    def service(self):
        with patch(
            "stock_datasource.modules.realtime_minute.service.get_cache_store"
        ) as mock_cache:
            mock_store = MagicMock()
            mock_cache.return_value = mock_store
            from stock_datasource.modules.realtime_minute.service import (
                RealtimeMinuteService,
            )
    ...
    def test_get_minute_data_fallback_to_clickhouse(self, service):
        service._cache.get_bars.return_value = []
        with patch(
            "stock_datasource.modules.realtime_minute.service._execute_query"
        ) as mock_query:
    ...
    def test_get_latest_minute(self, service):
        service._cache.get_latest.return_value = {
            "ts_code": "600519.SH",
            "close": 1810.0,
        }
...
class TestScheduler:
    ...
    def test_is_trading_time_during_morning(self):
        ...
        with patch(
            "stock_datasource.modules.realtime_minute.scheduler.datetime"
        ) as mock_dt:
    ...
    def test_run_collection(self):
        with (
            patch(
                "stock_datasource.modules.realtime_minute.collector.get_collector"
            ) as mock_get_collector,
            patch(
                "stock_datasource.modules.realtime_minute.cache_store.get_cache_store"
            ) as mock_get_cache,
        ):
    ...
    def test_run_sync(self):
        with patch(
            "stock_datasource.modules.realtime_minute.sync_service.get_sync_service"
        ) as mock_get_svc:
            mock_svc = MagicMock()
