...
def make_user(user_id: str = "user_001") -> dict:
    return {"id": user_id, "username": "testuser", "email": "test@test.com", "is_admin": False}


def make_profile_dict(profile_id: str = None, user_id: str = "user_001",
                      name: str = "默认账户", broker: str = "", is_default: bool = True):
    return {
...
class TestProfileCRUD:
    ...
    def test_list_profiles_returns_profiles_for_user(self):
        ...
        with patch('stock_datasource.models.database.db_client') as mock_db:
            ...
            mock_df = pd.DataFrame([
                {"id": "p1", "user_id": "user_001", "name": "中信", "broker": "", "is_default": 1},
                {"id": "p2", "user_id": "user_001", "name": "华泰", "broker": "华泰证券", "is_default": 0},
            ])
    ...
    def test_get_profile_by_id(self):
        ...
        with patch('stock_datasource.models.database.db_client') as mock_db:
            mock_df = pd.DataFrame([{
                "id": "p1", "user_id": "user_001", "name": "中信",
                "broker": "", "is_default": 1,
            }])
            mock_db.execute_query = Mock(return_value=mock_df)
    ...
    def test_delete_profile(self):
        ...
        with patch('stock_datasource.models.database.db_client') as mock_db:
            mock_df = pd.DataFrame([{
                "id": "p2", "user_id": "user_001", "name": "华泰",
                "broker": "", "is_default": 0,
            }])
            mock_db.execute_query = Mock(return_value=mock_df)
            mock_db.execute = Mock()
    ...
    def test_delete_default_profile_fails(self):
        ...
        with patch('stock_datasource.models.database.db_client') as mock_db:
            mock_df = pd.DataFrame([{
                "id": "p1", "user_id": "user_001", "name": "默认",
                "broker": "", "is_default": 1,
            }])
            mock_db.execute_query = Mock(return_value=mock_df)
...
class TestProfileScopedPositions:
    ...
    async def test_get_positions_filters_by_profile(self):
        ...
        with patch('stock_datasource.models.database.db_client') as mock_db:
            mock_df = pd.DataFrame([
                {"id": "pos1", "ts_code": "600519.SH", "stock_name": "贵州茅台",
                 "quantity": 100, "cost_price": 1700.0, "buy_date": "2026-01-01",
                 "current_price": None, "market_value": None, "profit_loss": None,
                 "profit_rate": None, "notes": "", "profile_id": "profile_001"},
            ])
            mock_db.execute_query = Mock(return_value=mock_df)
            ...
            positions = await svc.get_positions(user_id="user_001", profile_id="profile_001")
...
class TestProfileAPIEndpoints:
    ...
    def test_list_profiles_endpoint_requires_auth(self):
        ...
        assert response.status_code in (401, 403, 500)  # 500 if auth module not importable

    def test_create_profile_endpoint(self):
        """POST /api/portfolio/profiles should create a profile."""
        from stock_datasource.modules.profile.router import router, _get_current_user
        from fastapi import FastAPI
        from fastapi.testclient import TestClient
