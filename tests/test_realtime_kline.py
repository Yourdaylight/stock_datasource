...
class TestSchemas:
    ...
    def test_collect_status_response(self):
        from stock_datasource.modules.realtime_kline.schemas import (
...
class TestCacheStore:
    ...
    def cache_store(self):
        ...
        from stock_datasource.modules.realtime_kline.cache import (
            RealtimeKlineCacheStore,
        )
    ...
    def test_xadd_event(self, cache_store, mock_redis):
        ...
        eid = cache_store.xadd_event(
            "a_stock", {"ts_code": "000001.SZ", "payload": "{}"}
        )
        assert eid == "1234-0"
    ...
    def test_checkpoint_get_set(self, cache_store, mock_redis):
        ...
        assert (
            cache_store.get_checkpoint("stock:rtk:ckpt:clickhouse", "a_stock")
            == "999-0"
        )
    ...
    def test_push_to_dlq(self, cache_store, mock_redis):
        ok = cache_store.push_to_dlq(
            "stock:rtk:deadletter:push", "a_stock", {"test": 1}
        )
        assert ok is True
    ...
    def test_update_and_get_status(self, cache_store, mock_redis):
        ...
        mock_redis.hgetall.return_value = {
            "a_stock": json.dumps(
                {"last_collect_time": "2026-03-01 10:00:00", "records": 500}
            )
        }
...
class TestCollectorNormalize:
    def test_normalize_basic(self):
        from stock_datasource.modules.realtime_kline.collector import (
...
class TestCloudPushWorker:
    ...
    def push_worker(self):
        with patch("stock_datasource.config.settings.settings") as mock_s:
            ...
            with patch(
                "stock_datasource.modules.realtime_kline.cloud_push.get_cache_store"
            ) as mock_cache_fn:
                ...
                from stock_datasource.modules.realtime_kline.cloud_push import (
                    CloudPushWorker,
                )
    ...
    def test_tick_skipped_when_disabled(self, push_worker):
        with patch(
            "stock_datasource.modules.realtime_kline.cloud_push._is_push_enabled",
            return_value=False,
        ):
            push_worker.tick()
    ...
    def test_circuit_breaker_activation(self, push_worker):
        push_worker._settings.RT_KLINE_PUSH_CIRCUIT_BREAKER_MINUTES = (
            0.0001  # tiny for test
        )
...
class TestMinuteSinkWorker:
    ...
    def sink_worker(self):
        ...
        with patch(
            "stock_datasource.modules.realtime_kline.sync_service.get_cache_store"
        ) as mock_fn:
            mock_cache = MagicMock()
            ...
            from stock_datasource.modules.realtime_kline.sync_service import (
                MinuteSinkWorker,
            )
    ...
    def test_tick_no_data(self, sink_worker):
        with patch(
            "stock_datasource.modules.realtime_kline.sync_service._get_db"
        ) as mock_db:
...
class TestScheduler:
    ...
    def test_is_trading_time(self):
        ...
        with patch(
            "stock_datasource.modules.realtime_kline.scheduler.datetime"
        ) as mock_dt:
    ...
    def test_runtime_lifecycle(self):
        from stock_datasource.modules.realtime_kline.scheduler import (
            RealtimeKlineRuntime,
        )
    ...
    def test_runtime_start_stop(self):
        ...
        rt = RealtimeKlineRuntime()
        with (
            patch.object(rt, "_collector", MagicMock()),
    ...
    def test_run_collection(self):
        with (
            patch(
                "stock_datasource.modules.realtime_kline.collector.get_collector"
            ) as mock_gc,
            patch(
                "stock_datasource.modules.realtime_kline.cache.get_cache_store"
            ) as mock_cs,
        ):
    ...
    def test_start_push_if_needed(self):
        ...
        rt = RealtimeKlineRuntime()
        with patch(
            "stock_datasource.modules.realtime_kline.scheduler.PushThread"
        ) as MockPT:
...
class TestMetrics:
    ...
    def test_convenience_functions(self):
        ...
        assert (
            m_mod.metrics.counter(
                "rt_kline_collector_calls_total", labels={"market": "a_stock"}
            )
            == 1
        )
        assert (
            m_mod.metrics.counter(
                "rt_kline_collector_success_total", labels={"market": "a_stock"}
            )
            == 1
        )
        ...
        assert (
            m_mod.metrics.counter(
                "rt_kline_push_events_total", labels={"market": "hk", "status": "ok"}
            )
            == 1
        )
...
class TestService:
    ...
    def test_get_latest_from_redis(self):
        with patch(
            "stock_datasource.modules.realtime_kline.service.get_cache_store"
        ) as mock_fn:
            mock_cache = MagicMock()
            mock_fn.return_value = mock_cache
            mock_cache.get_latest.return_value = {"ts_code": "000001.SZ", "close": 10.0}

            from stock_datasource.modules.realtime_kline.service import (
    ...
    def test_get_latest_fallback_to_clickhouse(self):
        with (
            ...
            patch(
                "stock_datasource.modules.realtime_kline.service._execute_query"
            ) as mock_q,
        ):
            mock_cache = MagicMock()
            mock_fn.return_value = mock_cache
...
class TestRouter:
    ...
    def test_get_latest(self, client):
        with patch(
            "stock_datasource.modules.realtime_kline.router.get_realtime_kline_service"
        ) as mock_svc_fn:
            mock_svc = MagicMock()
    ...
    def test_runtime_health(self, client):
        with patch(
            "stock_datasource.modules.realtime_kline.scheduler.get_runtime"
        ) as mock_rt_fn:
            mock_rt = MagicMock()
            mock_rt_fn.return_value = mock_rt
            mock_rt.is_running = True
            mock_rt.health.return_value = {
                "collector": True,
                "push": False,
                "sink": True,
            }
    ...
    def test_trigger_collection(self, client):
        with patch(
            "stock_datasource.modules.realtime_kline.scheduler.run_collection"
        ) as mock_run:
    ...
    def test_trigger_sync(self, client):
        with patch(
            "stock_datasource.modules.realtime_kline.scheduler.run_sink_tick"
        ) as mock_run:
            mock_run.return_value = {
                "all_ok": True,
                "markets": {"a_stock": {"ok": True, "records": 50}},
            }
    ...
    def test_trigger_cleanup(self, client):
        with patch(
            "stock_datasource.modules.realtime_kline.scheduler.run_cleanup"
        ) as mock_run:
            mock_run.return_value = {
...
class TestDegradation:
    def test_push_timeout_records_failure(self):
        ...
        with patch("stock_datasource.config.settings.settings") as mock_s:
            ...
            with patch(
                ...
                worker._session.post.side_effect = requests.exceptions.Timeout(
                    "timeout"
                )
    ...
    def test_sink_single_market_failure_isolation(self):
        ...
        worker = MinuteSinkWorker()
        worker._cache = mock_cache
        worker._ensured_tables = set(
            [
                "ods_rt_kline_tick_cn",
                "ods_rt_kline_tick_etf",
                "ods_rt_kline_tick_index",
                "ods_rt_kline_tick_hk",
            ]
        )
