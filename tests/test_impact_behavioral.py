...
def _import_module_directly(module_name: str, rel_path: str):
    ...
    spec = importlib.util.spec_from_file_location(
        module_name, _src_dir / rel_path, submodule_search_locations=[]
    )
...
def _setup_minimal_deps():
    ...
    if "stock_datasource" not in sys.modules or not hasattr(
        sys.modules.get("stock_datasource", object), "__path__"
    ):
...
class TestCacheServiceReconnection:
    ...
    def _get_class(self):
        mod, spec = _import_module_directly(
            "cache_service_real", "stock_datasource/services/cache_service.py"
        )
        _setup_minimal_deps()
        try:
        ...
        for line in lines:
            ...
            if in_method:
                ...
                if "WHERE database = '" in line and 'f"' in line:
                    pytest.fail(
                        f"get_partition_info uses unsafe f-string: {line.strip()}"
                    )
...
class TestCutoffTimeSafety:
    ...
    def _read_source(self):
        return (
            _src_dir / "stock_datasource" / "modules" / "datamanage" / "service.py"
        ).read_text()
...
class TestKlineSqlSafety:
    ...
    def _read_etf_source(self):
        return (
            _src_dir / "stock_datasource" / "modules" / "etf" / "service.py"
        ).read_text()

    def _read_index_source(self):
        return (
            _src_dir / "stock_datasource" / "modules" / "index" / "service.py"
        ).read_text()
    ...
    def test_etf_kline_uses_literal_for_dates(self):
        ...
        for line in lines:
            ...
            if in_kline and "_to_clickhouse_literal(end_date)" in line:
                found_end = True
        assert found_start, (
            "ETF get_kline should use _to_clickhouse_literal(start_date)"
        )
        ...
        for i, line in enumerate(lines):
            ...
            if in_kline and "trade_date >=" in line:
                ...
                if 'f"' in line and "'{" in line:
                    pytest.fail(
                        f"Line {i + 1}: unsafe f-string date filter: {line.strip()}"
                    )
    ...
    def test_index_kline_uses_literal_for_ts_code(self):
        ...
        assert found_literal, (
            "Index get_kline should use _to_clickhouse_literal(ts_code)"
        )
    ...
    def test_index_kline_uses_literal_for_dates(self):
        ...
        for line in lines:
            ...
            if in_kline and "_to_clickhouse_literal(end_date)" in line:
                found_end = True
        assert found_start, (
            "Index get_kline should use _to_clickhouse_literal(start_date)"
        )
...
class TestTimeoutConsistency:
    ...
    def test_task_queue_default_timeout(self):
        source = (
            _src_dir / "stock_datasource" / "services" / "task_queue.py"
        ).read_text()
...
class TestSafeLimitValidation:
    ...
    def _get_service_class(self):
        ...
        _setup_minimal_deps()
        mod, spec = _import_module_directly(
            "service_real", "stock_datasource/modules/datamanage/service.py"
        )
...
class TestDeepAnalyzerParameterized:
    ...
    def _read_source(self):
        return (
            _src_dir / "stock_datasource" / "modules" / "quant" / "deep_analyzer.py"
        ).read_text()
    ...
    def test_get_stock_name_no_fstring_where(self):
        ...
        for i, line in enumerate(lines):
            ...
            if in_method and "WHERE ts_code = '" in line and 'f"' in line:
                pytest.fail(
                    f"Line {i + 1}: _get_stock_name uses unsafe f-string: {line.strip()}"
                )
...
class TestGetDailySqlSafety:
    ...
    def test_etf_get_daily_no_double_quote_escape(self):
        ...
        for i, line in enumerate(lines):
            ...
            if in_daily and 'replace("\'", "\'\'")' in line:
                pytest.fail(
                    f"Line {i + 1}: ETF get_daily still uses unsafe replace pattern"
                )
    ...
    def test_index_get_daily_uses_literal(self):
        ...
        for line in lines:
            ...
            if in_daily and "_to_clickhouse_literal(ts_code)" in line:
                found_literal = True
        assert found_literal, (
            "Index get_daily should use _to_clickhouse_literal(ts_code)"
        )
    ...
    def test_index_get_daily_no_double_quote_escape(self):
        ...
        for i, line in enumerate(lines):
            ...
            if in_daily and 'replace("\'", "\'\'")' in line:
                pytest.fail(
                    f"Line {i + 1}: Index get_daily still uses unsafe replace pattern"
                )
...
class TestEtfGetEtfsSqlSafety:
    ...
    def test_get_etfs_no_double_quote_escape(self):
        ...
        assert 'replace("\'", "\'\'")' not in source, (
            "etf/service.py should not use unsafe replace pattern"
        )
    ...
    def test_get_etfs_uses_literal_for_keyword_like(self):
        ...
        assert found_kw_literal, (
            "get_etfs LIKE filter should use kw_literal from _to_clickhouse_literal"
        )
...
class TestIndexSqlSafety:
    ...
    def test_no_double_quote_escape_anywhere(self):
        ...
        source = self._read_source()
        assert 'replace("\'", "\'\'")' not in source, (
            "index/service.py should not use unsafe replace pattern"
        )
    ...
    def test_get_indices_uses_literal_for_keyword_like(self):
        ...
        for line in lines:
            ...
            if in_method and "kw_literal" in line and "_to_clickhouse_literal" in line:
                found_kw_literal = True
        assert found_kw_literal, (
            "get_indices ILIKE filter should use kw_literal from _to_clickhouse_literal"
        )
...
class TestGetFactorsWhitelist:
    ...
    def test_indicators_filtered_by_whitelist(self):
        ...
        assert (
            "if i in _ALLOWED_INDICATORS" in source
            or "i in _ALLOWED_INDICATORS" in source
        )
    ...
    def test_no_raw_indicators_in_sql(self):
        ...
        for i, line in enumerate(lines):
            ...
            if in_method and "for i in indicators if i not in base_cols" in line:
                pytest.fail(
                    f"Line {i + 1}: get_factors uses indicators without whitelist filtering"
                )
...
class TestHttpRequestParameterRendering:
    ...
    def test_request_method_handles_params(self):
        ...
        assert found_param_rendering, (
            "_request should render params via _to_clickhouse_literal"
        )
        assert found_unreplaced_check, (
            "_request should validate no unreplaced placeholders remain"
        )
    ...
    def test_http_client_table_exists_uses_params(self):
        ...
        assert found_placeholder, (
            "ClickHouseHttpClient.table_exists should use %(database)s placeholder"
        )
...
class TestToClickhouseLiteralEdgeCases:
    ...
    def _get_func(self):
        mod, spec = _import_module_directly(
            "database_real", "stock_datasource/models/database.py"
        )
        _setup_minimal_deps()
    ...
    def test_no_inline_import_os(self):
        ...
        for i, line in enumerate(lines):
            ...
            if in_method and line.strip() == "import os":
                pytest.fail(
                    f"Line {i + 1}: redundant inline 'import os' in _run_task_with_timeout"
                )
    ...
    def test_no_inline_import_signal(self):
        ...
        for line in lines[:30]:
            if line.strip() == "import signal":
                ...
                break
        assert has_top_level_signal, (
            "task_worker.py should have top-level 'import signal'"
        )
        ...
        for i, line in enumerate(lines):
            ...
            if in_method and line.strip() == "import signal":
                pytest.fail(
                    f"Line {i + 1}: redundant inline 'import signal' in _run_task_with_timeout"
                )
