# A股赛博操盘手 🤖📈

**AI 原生的 A 股智能投资助手** —— 基于大语言模型和 LangGraph 多智能体架构，为个人投资者提供专业级的股票分析、智能选股、投资组合管理,策略回测,AI生成量化策略能力。

## ✨ 核心特性

### 🧠 AI 原生架构

- **多智能体协作**：8 个专业 Agent 分工协作，覆盖投资全流程
- **智能意图识别**：自动理解用户意图，路由到最合适的专业 Agent
- **自然语言交互**：用自然语言与系统对话，无需学习复杂操作
- **流式响应**：实时展示 AI 思考过程，体验更流畅
- **Langfuse 可观测**：完整的 AI 调用链路追踪和分析

### 🎯 八大专业 Agent

| Agent | 功能 | 典型场景 |
|-------|------|----------|
| **MarketAgent** | 行情分析 | "分析贵州茅台的走势"、"600519 估值如何" |
| **ScreenerAgent** | 智能选股 | "找出低估值高成长的股票"、"筛选股息率超过5%的股票" |
| **ReportAgent** | 财报分析 | "分析宁德时代的财务状况"、"比较茅台和五粮液的财报" |
| **PortfolioAgent** | 持仓管理 | "查看我的持仓"、"分析投资组合风险" |
| **BacktestAgent** | 策略回测 | "回测双均线策略"、"测试这个选股条件的历史收益" |
| **MemoryAgent** | 用户记忆 | "记住我的自选股"、"我的投资偏好是什么" |
| **DataManageAgent** | 数据管理 | "更新今日数据"、"检查数据质量" |
| **ChatAgent** | 通用对话 | 其他投资相关问题 |

### 📊 智能选股系统

- **实时行情展示**：分页展示全市场股票行情，支持排序和搜索
- **多维度筛选**：PE、PB、市值、涨跌幅、换手率等多条件组合
- **AI 辅助选股**：自然语言描述选股条件，AI 自动生成筛选策略
- **市场概览**：涨跌统计、成交额分布等市场全貌

### 📈 专业行情分析

- **K 线图表**：交互式 K 线图，支持多种技术指标
- **趋势分析**：均线系统、MACD、RSI 等技术分析
- **估值分析**：PE、PB、市值等基本面指标
- **对比分析**：多股票横向对比

### 💼 投资组合管理

- **持仓跟踪**：实时计算持仓盈亏
- **风险分析**：组合波动率、最大回撤等风险指标
- **收益归因**：分析收益来源
- **交易记录**：完整的买卖记录管理

### 🔄 策略回测

- **可视化回测**：图表展示策略表现
- **多策略支持**：均线、动量、价值等多种策略模板
- **详细报告**：收益率、夏普比率、最大回撤等关键指标
- **参数优化**：自动寻找最优策略参数

## 🚀 快速开始

### 1. 环境准备

```bash
# 克隆项目
git clone <repository-url>
cd stock_datasource

# 安装依赖
uv sync

# 配置环境变量
cp .env.example .env
```

编辑 `.env` 文件：
```env
# 必需：TuShare API Token
TUSHARE_TOKEN=your_token_here

# 必需：OpenAI API 配置（AI 功能）
OPENAI_API_KEY=your_api_key_here
OPENAI_BASE_URL=https://api.openai.com/v1
OPENAI_MODEL=gpt-4

# ClickHouse 配置
CLICKHOUSE_HOST=localhost
CLICKHOUSE_PORT=9000
CLICKHOUSE_DATABASE=stock_datasource
```

### 2. 初始化数据库

```bash
uv run cli.py init-db
```

### 3. 采集数据

```bash
# 加载股票基础信息
uv run cli.py load-stock-basic

# 加载交易日历
uv run cli.py load-trade-calendar --start-date 20250101 --end-date 20251231

# 采集日线数据
uv run cli.py ingest-daily --date 20251024
```

### 4. 启动服务

```bash
# 启动后端 API 服务
uv run python -m stock_datasource.services.http_server

# 启动前端（新终端）
cd frontend && npm install && npm run dev
```

访问 http://localhost:5173 开始使用！

## 🏗️ 系统架构

```
┌─────────────────────────────────────────────────────────────────┐
│                         前端 (Vue 3 + TypeScript)                │
│  ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐ ┌─────────┐   │
│  │ 智能对话 │ │ 智能选股 │ │ 行情分析 │ │ 持仓管理 │ │ 策略回测 │   │
│  └────┬────┘ └────┬────┘ └────┬────┘ └────┬────┘ └────┬────┘   │
└───────┼──────────┼──────────┼──────────┼──────────┼──────────┘
        │          │          │          │          │
        ▼          ▼          ▼          ▼          ▼
┌─────────────────────────────────────────────────────────────────┐
│                      API Layer (FastAPI)                         │
│  ┌──────────────────────────────────────────────────────────┐   │
│  │                  OrchestratorAgent                        │   │
│  │    ┌──────────────────────────────────────────────┐      │   │
│  │    │           意图识别 & 路由分发                   │      │   │
│  │    └──────────────────────────────────────────────┘      │   │
│  │         │         │         │         │         │         │   │
│  │    ┌────▼───┐ ┌───▼────┐ ┌──▼───┐ ┌──▼───┐ ┌───▼────┐   │   │
│  │    │ Market │ │Screener│ │Report│ │Portfo│ │Backtest│   │   │
│  │    │ Agent  │ │ Agent  │ │Agent │ │Agent │ │ Agent  │   │   │
│  │    └────────┘ └────────┘ └──────┘ └──────┘ └────────┘   │   │
│  └──────────────────────────────────────────────────────────┘   │
└─────────────────────────────────────────────────────────────────┘
        │                                                    │
        ▼                                                    ▼
┌───────────────────┐                          ┌───────────────────┐
│   LLM (OpenAI)    │                          │  ClickHouse DB    │
│   GPT-4 / GPT-4o  │                          │  A股全量数据       │
└───────────────────┘                          └───────────────────┘
```

### 技术栈

**后端**
- Python 3.11+ / FastAPI / LangGraph
- ClickHouse (高性能列式数据库)
- TuShare (A股数据源)
- Langfuse (AI 可观测性)

**前端**
- Vue 3 + TypeScript
- TDesign 组件库
- ECharts (K线图表)
- Pinia (状态管理)

**AI**
- LangGraph (多智能体框架)
- OpenAI GPT-4 / GPT-4o
- Function Calling (工具调用)

## 📁 项目结构

```
stock_datasource/
├── src/stock_datasource/
│   ├── agents/              # AI Agent 层
│   │   ├── orchestrator.py  # 编排器
│   │   ├── market_agent.py  # 行情分析
│   │   ├── screener_agent.py # 智能选股
│   │   ├── report_agent.py  # 财报分析
│   │   ├── portfolio_agent.py # 持仓管理
│   │   ├── backtest_agent.py # 策略回测
│   │   ├── memory_agent.py  # 用户记忆
│   │   └── tools.py         # Agent 工具集
│   ├── plugins/             # 数据采集插件
│   ├── services/            # HTTP/MCP 服务
│   ├── modules/             # 功能模块
│   └── core/                # 核心组件
├── frontend/                # Vue 前端
├── docs/                    # 文档
├── cli.py                   # 命令行工具
└── tests/                   # 测试
```

## 🔌 MCP Server 支持

系统提供 MCP (Model Context Protocol) Server，可集成到 Claude Code、Cursor 等 AI IDE：

```json
{
  "mcpServers": {
    "stock_datasource": {
      "url": "http://localhost:8001/messages",
      "transport": "streamable-http"
    }
  }
}
```

启动 MCP Server：
```bash
uv run python -m stock_datasource.services.mcp_server
```

## 📚 文档

| 文档 | 说明 |
|------|------|
| [CLI 使用指南](docs/CLI_GUIDE.md) | 命令行工具详细使用说明 |
| [开发指南](DEVELOPMENT_GUIDE.md) | 开发者文档 |
| [插件开发](PLUGIN_QUICK_START.md) | 新建数据插件快速参考 |

## 🧪 测试

```bash
# 运行所有测试
uv run pytest tests/

# 测试 AI Agent
uv run python -c "
from dotenv import load_dotenv; load_dotenv()
from stock_datasource.agents import get_orchestrator
import asyncio

async def test():
    orch = get_orchestrator()
    result = await orch.execute('分析贵州茅台')
    print(result.response)

asyncio.run(test())
"
```

## 📄 许可证

MIT License

## 🤝 贡献

欢迎提交 Issue 和 Pull Request！

1. Fork 本仓库
2. 创建特性分支 (`git checkout -b feature/AmazingFeature`)
3. 提交更改 (`git commit -m 'Add some AmazingFeature'`)
4. 推送到分支 (`git push origin feature/AmazingFeature`)
5. 开启 Pull Request
